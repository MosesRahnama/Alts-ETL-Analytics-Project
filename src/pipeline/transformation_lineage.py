...
from src.common import matrices as decision_matrices

RECEIPT_RULES = "receipt-note-policy"

NOTE_LIMIT = int(decision_matrices.resolve(RECEIPT_RULES, "note_limit"))
assert NOTE_LIMIT == 2000
...
def _receipt_id(
    # receipt-note-policy.csv names what the id digests.
    stage_id: str,
    command: str,
...
def receipt_errors(
    *,
    ...
    Two different things were reported as one failure. An archived object whose
    bytes differ from the hash the receipt records is an integrity failure: the
    record and the artifact disagree, and the release cannot ship. An object
    the archive no longer holds at all is incomplete historic restoration: the
    receipt still names the bytes it wrote, every current output still rebuilds
    from its inputs, and nothing about the release is contradicted.

    Pass a list as `gaps` to collect the absent objects there instead of into
    the errors. Each entry is `(receipt_id, sha256, recorded_path)`, carrying
    the hash of the object that is missing rather than the receipt it came
    from, so a caller can tell an absent input from an absent output. The
    caller then decides, and `main` reports them apart."""
        ...
        for item in input_items:
            ...
            if not target.is_file():
                if gaps is None:
                    ...
                    gaps.append((receipt_id, expected, input_path))
        ...
        for hash_field, path_field in (
            ...
            if not target.is_file():
                if gaps is None:
                    ...
                    gaps.append((receipt_id, expected, row.get("output_path", "")))
            elif digest(target) != expected:
                errors.append(f"{receipt_id}: archived object hash differs: {object_path}")
    return errors


def restore_archive_from_live_files(
    *,
    receipt_path: Path = RECEIPT_PATH,
    root: Path = PROJECT_ROOT,
    archive_dir: Path = ARCHIVE_DIR,
) -> tuple[int, int]:
    """Put back every archive object whose bytes are still a file in the tree.

    A receipt records the hash of what it read and wrote. Where the archive has
    lost that object and a file in the repository still hashes to it, the bytes
    are recoverable and there is nothing to decide: copy them back under their
    own address. Returns the number restored and the number still absent."""
    ...
    archive_dir.mkdir(parents=True, exist_ok=True)
    # One digest per distinct file, not one per gap. The candidate files run to
    # hundreds of megabytes of DuckDB and the gaps name them repeatedly, so
    # hashing per gap read the same databases dozens of times.
    live: dict[str, str] = {}
    for _, _, relative in gaps:
        if relative and relative not in live:
            path = root / relative
            live[relative] = digest(path) if path.is_file() else ""

    by_hash: dict[str, str] = {}
    for _, sha, relative in gaps:
        if relative and live.get(relative) == sha:
            by_hash.setdefault(sha, relative)

    restored = 0
    absent = 0
    for sha in sorted({sha for _, sha, _ in gaps}):
        relative = by_hash.get(sha)
        if relative is None:
            absent += 1
            continue
        source = root / relative
        shutil.copyfile(source, archive_dir / f"sha256-{sha}{source.suffix.casefold() or '.bin'}")
        restored += 1
    return restored, absent


def live_output_gaps(
    gaps: Iterable[tuple[str, str, str]],
    *,
    root: Path = PROJECT_ROOT,
) -> list[str]:
    """The archive gaps whose bytes a file in the tree still carries today.

    A gap on a superseded state costs the release nothing: the current file is
    present and rebuilds from its recorded inputs. A gap whose hash matches a
    file sitting in the tree is recoverable, and leaving it absent means the
    receipt for a shipped file has no archived copy while the bytes are right
    there, so that one stays a failure until `--restore-archive` puts it back.

    The hash comes from the gap itself. Reading it off the receipt instead
    attributed a missing *input* to the receipt's output, which reported
    thirty-nine recoverable gaps where the restore found none."""

    live: dict[str, str] = {}
    failures = []
    for receipt_id, sha, relative in gaps:
        if not relative:
            continue
        if relative not in live:
            path = root / relative
            live[relative] = digest(path) if path.is_file() else ""
        if live[relative] == sha:
            failures.append(
                f"{receipt_id}: the archive lacks the bytes of the live {relative}"
            )
    return failures
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--restore-archive",
        action="store_true",
        help="Copy back every archive object whose bytes are still a file in the tree.",
    )
    args = parser.parse_args(argv)
    if args.restore_archive:
        restored, absent = restore_archive_from_live_files(
            receipt_path=args.receipt_path, archive_dir=args.archive_dir
        )
        print(f"RESTORED: {restored} objects copied back; {absent} remain superseded and absent")
        return 0
    ...
    errors.extend(live_output_gaps(gaps))
    if errors:
        # The first ten used to be the whole report, and an auditor reading it
        # counted ten missing objects where the ledger referenced eighty-one.
        # The remainder is now stated even when it is not listed.
        shown = "; ".join(errors[:10])
        rest = f" (+{len(errors) - 10} more)" if len(errors) > 10 else ""
        print(f"FAIL: {shown}{rest}")
        return 1
    mode = "receipt structure" if args.structure_only else "external artifact archive"
    if gaps:
        objects = {sha for _, sha, _ in gaps}
        print(
            f"GAP: the external archive no longer holds {len(objects)} superseded objects"
            f" named by {len(gaps)} receipt references; every current output is present"
            " and rebuilds from its recorded inputs"
        )
