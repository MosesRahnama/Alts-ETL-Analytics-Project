# Statements and Economics: EXTRACTOR A
...
- **Record output:** `ledgers/working/pdf-extraction-csv/06-statements-and-economics/<file_id>/records-a.csv`
- **Coverage output:** `ledgers/working/pdf-extraction-csv/06-statements-and-economics/<file_id>/coverage-a.csv`
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py lane-check --route 06-statements-and-economics --agent A
```
...
|---|---|---|---|---|
| `Schedule_Inv` | `CORE` | `document_context` | one row per document | none |
| `Schedule_Inv` | `CORE` | `position_observation` | one populated allowed source value cell for one named position | `quantity`, `cost`, `fair_value`, `market_value`, `notional`, `portfolio_weight`, `interest_rate`, `maturity_date` |
| `Schedule_Inv` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
| `Fee_Report` | `CORE` | `document_context` | one row per document | none |
| `Fee_Report` | `CORE` | `fee_observation` | one populated allowed source value cell | `management_fee`, `performance_fee`, `cost_bps`, `offset`, `fee_benchmark`, `nav_aum_denominator` |
| `Fee_Report` | `CORE` | `fund_economics_observation` | one populated allowed source value cell | `commitment`, `paid_in_capital`, `paid_in_capital_multiple`, `contribution`, `distribution`, `nav`, `unfunded_commitment`, `recallable_distribution`, `tvpi`, `dpi`, `rvpi`, `moic`, `ownership_percentage`, `income`, `fee`, `carried_interest` |
| `Fee_Report` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
| `Valuation` | `CORE` | `document_context` | one row per document | none |
| `Valuation` | `CORE` | `valuation_observation` | one printed valuation fact or one populated value cell | `method`, `frequency`, `valuer`, `oversight`, `independent_review`, `enterprise_value` |
| `Valuation` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
| `NAV_Statement` | `CORE` | `document_context` | one row per document | none |
| `NAV_Statement` | `CORE` | `nav_observation` | one populated allowed source value cell | `nav_per_share`, `shares_units`, `transaction_price`, `nav_component`, `repurchase_limit`, `request_satisfaction`, `valuation_assumption`, `valuation_sensitivity` |
| `NAV_Statement` | `CORE` | `valuation_observation` | one printed valuation fact or one populated value cell | `method`, `frequency`, `valuer`, `oversight`, `independent_review`, `enterprise_value` |
| `NAV_Statement` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
| `Cash_Flow_Notice` | `CORE` | `document_context` | one row per document | none |
| `Cash_Flow_Notice` | `CORE` | `cash_flow_observation` | one populated allowed source value cell or one notice component the page states in words | `capital_call`, `return_of_capital`, `preferred_return`, `expense`, `interest`, `net_cash_flow` |
| `Cash_Flow_Notice` | `CORE` | `fund_economics_observation` | one populated allowed source value cell | `commitment`, `paid_in_capital`, `paid_in_capital_multiple`, `contribution`, `distribution`, `nav`, `unfunded_commitment`, `recallable_distribution`, `tvpi`, `dpi`, `rvpi`, `moic`, `ownership_percentage`, `income`, `fee`, `carried_interest` |
| `Cash_Flow_Notice` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
| `PCAP` | `CORE` | `document_context` | one row per document | none |
| `PCAP` | `CORE` | `fund_economics_observation` | one populated allowed source value cell | `commitment`, `paid_in_capital`, `paid_in_capital_multiple`, `contribution`, `distribution`, `nav`, `unfunded_commitment`, `recallable_distribution`, `tvpi`, `dpi`, `rvpi`, `moic`, `ownership_percentage`, `income`, `fee`, `carried_interest` |
| `PCAP` | `CORE` | `cash_flow_observation` | one populated allowed source value cell or one notice component the page states in words | `capital_call`, `return_of_capital`, `preferred_return`, `expense`, `interest`, `net_cash_flow` |
| `PCAP` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
- **Schedule_Inv:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.
- **Fee_Report:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.
- **Valuation:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.
- **NAV_Statement:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.
- **Cash_Flow_Notice:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.
- **PCAP:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.
...
| Fees, carried interest, partnership expenses, offsets, rebates, cost ratios, or a fee rate and the basis it is charged on | `fee_observation` | every column in that table |
...
| Valuation results and how they were produced: method, frequency, valuer, oversight, independent review, resulting marks | `valuation_observation` | every column in that table |
| Net asset value and its components: NAV per share or unit, shares outstanding, repurchase or liquidity limits | `nav_observation` | every column in that table |
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py claim --route 06-statements-and-economics --agent A --model "<model name>"
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 06-statements-and-economics --file <file_id> --agent A
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 06-statements-and-economics --file <file_id> --agent A --through-page 3
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py audit-file --route 06-statements-and-economics --file <file_id> --agent A
```
...
EXTRACTION CHECKLIST, 22 required rules:
 1. Family follows the table, not the document type. One table, one
    family. This route may only use: position_observation, definition_context, fee_observation, fund_economics_observation, valuation_observation, nav_observation, cash_flow_observation.
