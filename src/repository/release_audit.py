...
def files_and_directories(root: Path) -> tuple[list[Path], list[Path]]:
    """The paths this audit judges: exactly the paths the manifest lists.

    This walked the tree itself with the manifest's directory exclusions and
    local-tree rule but without its Git filter, so every path .gitignore covers
    by pattern was audited against a manifest that deliberately omits it. The
    553 rendered page images and the render log were each reported as absent
    from the manifest, which is 554 of the audit's findings describing a
    disagreement between two walkers rather than anything about the release.

    Whether the 311 published-slice images should be tracked through Git LFS so
    a reviewer receives the pictures the image manifest describes is a
    repository-boundary decision recorded in .gitignore; it is not this
    function's to make, and it changes what `manifest_paths` returns rather
    than how the audit reads it."""

    entries = manifest_paths(root)
    files = sorted(path for path in entries if path.is_file() and not path.name.endswith(".part"))
...
def check_file(path: Path, root: Path) -> FileResult:
    ...
    if matrices.resolve_or_star("file-type-checks", suffix) not in {"csv_check", "database_check", "generic_check"}:
        raise RuntimeError("file-type-checks.csv names an unknown checker")
    try:
        if suffix == ".csv":
...
# The matrix the presentation check reads. Four contexts: `presentation` and
# `timeline` carry one regular expression per row with the label a hit prints;
# `presentation_target` and `timeline_target` carry the glob each set of
# patterns is applied to, relative to the project root.
PRESENTATION_PATTERNS = "presentation-patterns"


def presentation_patterns(context: str) -> dict[re.Pattern[str], str]:
    """The compiled patterns one context of the matrix carries, with their labels.

    A row that does not compile is named with its context and the error,
    because the audit's contract is one FAIL line and not a traceback."""

    compiled: dict[re.Pattern[str], str] = {}
    for row in matrices.rows_in(PRESENTATION_PATTERNS, context):
        try:
            pattern = re.compile(row["input_value"], flags=re.I)
        except re.error as exc:
            raise AuditError(
                f"{PRESENTATION_PATTERNS}.csv: {context} pattern {row['input_value']!r} "
                f"does not compile: {exc}"
            ) from exc
        compiled[pattern] = row["output_value"]
    if not compiled:
        raise AuditError(f"{PRESENTATION_PATTERNS}.csv carries no {context} rows")
    return compiled


def presentation_targets(root: Path, context: str) -> set[Path]:
    """The files one target context of the matrix names, by glob under the root."""

    targets: set[Path] = set()
    for pattern in matrices.mapping(PRESENTATION_PATTERNS, context):
...
def presentation_findings(root: Path) -> list[Finding]:
    ...
    Every pattern and every file list is a row of the presentation-patterns
    matrix, so editing the matrix changes what the audit refuses; nothing here
    is compared with a literal copy of it."""
    findings: list[Finding] = []
    patterns = presentation_patterns("presentation")
    timeline_patterns = presentation_patterns("timeline")
    targets = presentation_targets(root, "presentation_target")
    timeline_targets = presentation_targets(root, "timeline_target")
    for path in sorted(targets | timeline_targets):
