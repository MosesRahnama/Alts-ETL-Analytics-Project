...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 05-fund-legal-docs --file <file_id> --agent B --through-page 1
```
...
RE-READ BEFORE THE NEXT DOCUMENT, the fifteen rules that decay:
 1. Family follows the table, not the document type. One table, one
    family. This route may only use: legal_term, legal_clause, subscription_reference, ddq_quantitative_observation.
