...
# The loader's three decisions are rows in matrices under
PARITY_NULLS = "load-null-policy"
LOAD_NULLS = "database-null-policy"
MISSING_INPUTS = "database-missing-input-policy"


NULLSTR = matrices.resolve(LOAD_NULLS, "nullstr")
EMPTY_CELL_POLICY = matrices.resolve(PARITY_NULLS, "empty_cell_means")
MISSING_INPUT_POLICY = matrices.resolve(MISSING_INPUTS, "missing_input")
...
def read_csv(path: Path) -> tuple[list[str], list[list[str | None]]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        headers = next(reader)
        if EMPTY_CELL_POLICY != "null":
            raise matrices.MatrixError(
                f"{PARITY_NULLS}: unsupported empty_cell_means {EMPTY_CELL_POLICY!r}"
            )
        rows = [[value if value != NULLSTR else None for value in row] for row in reader]
    return headers, rows
...
def load(
    ...
    try:
        ...
        for table, filename in TABLE_FILES.items():
            ...
            if not path.exists():
                if MISSING_INPUT_POLICY == "skipped_without_clearing":
                    counts[table] = 0
                    continue
                raise matrices.MatrixError(
                    f"{MISSING_INPUTS}: unsupported missing_input policy {MISSING_INPUT_POLICY!r}"
                )
