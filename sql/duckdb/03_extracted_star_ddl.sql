...
CREATE TABLE IF NOT EXISTS fact_observation (
    ...
    metric_category         VARCHAR,
    metric_name             VARCHAR,
    -- A printed footnote or methodology note measures nothing, so a
    -- definition_context row has no subject. Every measure row still
    -- carries one; the CHECK below still closes the vocabulary.
    ...
    sector                  VARCHAR,
    -- Derived at flatten from context-grouping-map.csv: what the printed
    -- grouping is under one taxonomy, and whether the row states money.
    ...
    -- closed-vocabularies.csv, context value_kind
    CHECK (value_kind IN ('number', 'currency', 'percent', 'multiple', 'text', 'none')),
    -- closed-vocabularies.csv, context unit_scale
    ...
    CHECK (subject_type IS NULL OR subject_type IN ('document', 'reporting_entity', 'fund', 'portfolio', 'investment', 'manager', 'investor', 'asset_class', 'benchmark', 'peer_group', 'market_series', 'fee_scope', 'cash_flow', 'valuation_subject', 'foundation', 'program_related_investment', 'service_provider', 'clause_party', 'subscription', 'other_printed_scope')),
    -- closed-vocabularies.csv, context adjudication_status
    CHECK (adjudication_status IN ('RESOLVED', 'ADDED', 'VERIFIED_ONE_SIDED', 'AGREED')),
...
CREATE TABLE IF NOT EXISTS fact_holding (
    ...
    subject_type            VARCHAR,
    -- The industry the schedule printed beside the position; promotion reads
    -- fund_holdings.sector from here.
    sector                  VARCHAR,
