...
| Extraction results | [`document-summary.csv`](../data/extracted/review/document-summary.csv) | Per-document A/B rows, physical pairs, agreement, conflicts, decisions, and final rows |
| Observation origin record | [`trace-sample.csv`](../data/extracted/review/trace-sample.csv), [`observation origin CSV`](../data/extracted/review/observation%2Dlineage.csv) | A printed value through both proposals, comparison, resolution, final row, and fact row |
...
| Completion | [`reviewer cell-origin CSV`](../data/extracted/review/reviewer%2Dcell%2Dlineage.csv), [`reviewer-gap-ledger.csv`](../data/extracted/review/reviewer-gap-ledger.csv) | Source, formula, or parameter for each added cell |
| Source-only analytics | [`fund_metrics.csv`](../data/extracted/fund-level/fund_metrics.csv) | Multiples on qualifying printed periods, all `EXTRACTED` |
| Completed analytics | [`fund_metrics.csv`](../data/csv/fund_metrics.csv), [`pme_results.csv`](../data/csv/pme_results.csv) | Completed metrics and market comparisons, all `SYNTHETIC` |
| Release audit | [`FINAL-RELEASE-AUDIT.csv`](../docs/FINAL-RELEASE-AUDIT.csv) | Current stages, source controls, and closing checks |
...
Each matrix-backed data decision cites one or more rows under [`../data/normalization/transformations/`](../data/normalization/transformations/). The rows record the input, output, decision owner, context, and former code location. [`src/common/matrices.py`](../src/common/matrices.py) provides the shared reader; [`matrix-check.csv`](../ledgers/pipeline/matrix-check.csv) records every matrix per release, and [`migration-dispositions.csv`](../ledgers/pipeline/migration-dispositions.csv) records the four non-data structure sites.

## Release checks

| Command | Result |
|---|---|
| `python -m src.pipeline.publish_review_release` | Runs the ordered release stages with input/output receipts |
