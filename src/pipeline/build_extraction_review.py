...
# The review builder's decisions read from matrices under
# data/normalization/transformations.
IGNORED_FIELDS = "lineage-ignored-fields"
OUTPUT_DEFAULTS = "review-output-defaults"
LINEAGE_MAP = "review-lineage-field-map"
FACT_KEYS = "review-fact-key-fields"
INCLUSION = "review-document-inclusion"
DECISIONS = "decision-vocabulary"
SELECTION = "review-selection-policy"
SEMANTICS = "adjudication-decision-semantics"
METADATA_DUPES = "source-metadata-duplicate-policy"
PATH_PARSING = "source-path-parsing"
PATH_SELECTION = "source-path-selection"
RESOLUTION_ROUTING = "adjudication-resolution-routing"
RESOLUTION_DUPES = "adjudication-duplicate-policy"
LIST_PARSING = "review-list-parsing"
SUMMARY_FIELDS = "document-summary-field-policy"
LINEAGE_MATCHING = "review-lineage-match-policy"
FACT_MATCHING = "review-fact-match-policy"
SUMMARY_FORMULAS = "review-summary-formulas"
AGREEMENT = "agreement-definition"
STATUS_COLUMNS = "review-status-to-summary-column"
PAGE_STATUSES = "page-status-classification"
DISAGREEMENTS = "review-disagreement-aggregation"
TRACE_ORDER = "review-trace-sampling"

SIGNATURE_IGNORED = set(matrices.mapping(IGNORED_FIELDS))

# Every file build() writes into the review directory. The release declares
# this list, so a file added to build_outputs is receipted with the rest
# instead of reaching the dashboard with no hash behind it.
REVIEW_OUTPUT_NAMES: tuple[str, ...] = (
    "document-summary.csv",
    "label-category-ambiguity.csv",
...
def table_lineage_text(review_lineage_text: str) -> str:
    ...
    renamed_source = next(
        row["input_value"]
        for row in matrices.load(LINEAGE_MAP)
        if row["input_value"] != row["output_value"] and row["input_value"] != "passthrough_fields"
    )
    rows = (
        {
            "observation_id": row.get("observation_id", ""),
            "document_id": row.get(renamed_source, ""),
    ...
    if matrices.resolve(OUTPUT_DEFAULTS, "review_quoting") != "quote_all":
        raise ReviewBuildError("review-output-defaults.csv names an unknown quoting rule")
...
_PUBLISHED_KEY = tuple(
    row["output_value"]
    for row in sorted(matrices.rows_in(FACT_KEYS, "published"), key=lambda item: int(item["input_value"]))
)
_WORKING_KEY = (matrices.resolve(FACT_KEYS, "1", context="working"), *_PUBLISHED_KEY[1:])


def fact_key(row: Mapping[str, str], *, published: bool) -> tuple[str, ...]:
    fields = _PUBLISHED_KEY if published else _WORKING_KEY
...
def data_folders(root: Path) -> list[Path]:
    if matrices.resolve(INCLUSION, "included_folders") != "route_slash_document":
        raise ReviewBuildError("review-document-inclusion.csv names an unknown walk")
    return sorted(
        folder
...
def selected_record(
    ...
    if pair.get("requires_review") == "NO" and resolution is None:
        if matrices.resolve(SELECTION, "automatic_pair") != "a_row_confirmed" or (
            a_row is None or b_row is None or pair.get("pair_status") != "EXACT"
        ):
    ...
    if decision not in set(matrices.resolve(DECISIONS, "review_decisions", context="review").split("|")):
        raise ReviewBuildError(f"Unknown decision {decision!r} for {pair.get('pair_id')}")
    reason = resolution.get("reason", "")
    if matrices.resolve(SEMANTICS, decision) == "no_record":
...
def build_outputs(
    ...
    if matrices.resolve(METADATA_DUPES, "repeated_file_id") != "last_row_wins":
        raise ReviewBuildError("source-metadata-duplicate-policy.csv names an unknown rule")
    ...
    for folder in data_folders(working_root):
        ...
        if matrices.resolve(PATH_PARSING, "query_string") != "stripped":
            raise ReviewBuildError("source-path-parsing.csv names an unknown rule")
        ...
        if matrices.resolve(PATH_SELECTION, "candidate_priority") != "routing_then_ledger":
            raise ReviewBuildError("source-path-selection.csv names an unknown priority")
        source_candidates = route_candidates if len(route_candidates) == 1 else ledger_candidates
        source_pdf_path = (
            f"data/documents/pdf/{source_candidates[0]}"
            if len(source_candidates) == 1
            and matrices.resolve(PATH_SELECTION, "single_candidate") == "used"
            else ""
        )
        ...
        for number, row in enumerate(resolution_rows, 1):
            if (
                row.get("decision") == "ADD"
                and matrices.resolve(RESOLUTION_ROUTING, "ADD") == "addition_list"
            ):
                additions.append((number, row))
            elif row.get("pair_id"):
                if matrices.resolve(RESOLUTION_DUPES, "repeated_pair_id") != "last_row_wins":
                    raise ReviewBuildError("adjudication-duplicate-policy.csv names an unknown rule")
        ...
        for pair in pairs:
            ...
            for field in filter(
                None,
                pair.get("difference_fields", "").split(
                    matrices.resolve(LIST_PARSING, "difference_fields_separator")
                ),
            ):
                key = (status, field)
        ...
        for final_number, final in enumerate(final_rows, 1):
            if matrices.resolve(SUMMARY_FIELDS, "source_sha256") == "last_non_blank":
                source_sha256 = final.get("source_sha256", source_sha256)
            candidates = expected.get(signature(final), [])
            order = tuple(
                row["output_value"]
                for row in sorted(matrices.load(LINEAGE_MATCHING), key=lambda item: int(item["input_value"]))
            )
            if order != ("signature_match", "physical_key_match"):
                raise ReviewBuildError("review-lineage-match-policy.csv names an unknown order")
            ...
            if len(matching_facts) != 1:
                if matrices.resolve(FACT_MATCHING, "2") != "normalized_fact_key":
                    raise ReviewBuildError("review-fact-match-policy.csv names an unknown fallback")
        ...
        if matrices.resolve(SUMMARY_FORMULAS, "physical_pairs") != "pairs_minus_one_sided":
            raise ReviewBuildError("review-summary-formulas.csv names an unknown pair formula")
        physical_pairs = len(pairs) - pair_counts["A_ONLY"] - pair_counts["B_ONLY"]
        if matrices.resolve(AGREEMENT, "raw_value_agreement") != "same_printed_value_both_lanes":
            raise ReviewBuildError("agreement-definition.csv names an unknown definition")
        ...
        document_rows.append(
            {
                ...
                "pages_with_data": sum(
                    row.get("page_status") == matrices.resolve(PAGE_STATUSES, "pages_with_data_status")
                    for row in coverage
                ),
                ...
                "raw_value_agreement_rate": format(
                    value_agreements / physical_pairs,
                    matrices.resolve(SUMMARY_FORMULAS, "agreement_rate_format"),
                )
                if physical_pairs
                else "",
                **{
                    row["output_value"]: pair_counts[row["input_value"]]
                    for row in matrices.load(STATUS_COLUMNS)
                },
    ...
    if matrices.resolve(DISAGREEMENTS, "grouping") != "status_and_field":
        raise ReviewBuildError("review-disagreement-aggregation.csv names an unknown grouping")
    disagreement_rows = [
        {
            "pair_status": status,
            "field_name": field,
    ...
    preferred = {
        row["input_value"]: int(row["output_value"]) - 1
        for row in matrices.load(TRACE_ORDER)
    }
    ...
    # One printed label mapped to two categories is where reader judgement
    # diverges; the report lists every such label so an adjudicator settles it
    # once instead of per document. It is written every run and never fails.
    label_map: dict[str, dict[str, object]] = {}
    for fact in facts:
        label = " ".join(fact.get("metric_name", "").split()).casefold()
        if not label:
            continue
        entry = label_map.setdefault(
            label,
            {"label": fact.get("metric_name", ""), "categories": set(), "families": set(),
             "documents": set(), "rows": 0, "example": fact.get("observation_id", "")},
        )
        category = fact.get("metric_category", "") or fact.get("term_category", "")
        if category:
            entry["categories"].add(category)
        entry["families"].add(fact.get("record_family", ""))
        entry["documents"].add(fact.get("document_id", ""))
        entry["rows"] = int(entry["rows"]) + 1
    ambiguity_rows = [
        {
            "metric_name": entry["label"],
            "categories": " | ".join(sorted(entry["categories"])),
            "record_families": " | ".join(sorted(entry["families"])),
            "row_count": entry["rows"],
            "document_count": len(entry["documents"]),
            "example_observation_id": entry["example"],
        }
        for entry in sorted(label_map.values(), key=lambda item: (-len(item["categories"]), str(item["label"])))
        if len(entry["categories"]) > 1
    ]

    return {
        "document-summary.csv": csv_text(DOCUMENT_COLUMNS, document_rows),
        "label-category-ambiguity.csv": csv_text(
            ("metric_name", "categories", "record_families", "row_count",
             "document_count", "example_observation_id"),
            ambiguity_rows,
        ),
...
def build(
    ...
    if tuple(outputs) != REVIEW_OUTPUT_NAMES:
        raise ReviewBuildError(
            "build_outputs and REVIEW_OUTPUT_NAMES disagree: "
            f"{', '.join(sorted(set(outputs) ^ set(REVIEW_OUTPUT_NAMES)))}"
        )
    counts: dict[str, int] = {}
