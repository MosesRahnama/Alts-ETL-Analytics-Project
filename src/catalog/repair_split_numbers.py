...
SPLIT_RULES = "split-number-rules"

GAP_EPSILON = float(matrices.resolve(SPLIT_RULES, "gap_epsilon_points"))
ROW_BUCKET = float(matrices.resolve(SPLIT_RULES, "row_bucket_points"))
assert (GAP_EPSILON, ROW_BUCKET) == (0.6, 3.0)
NUM_TAIL = re.compile(r"[\d,.]$")
NUM_HEAD = re.compile(r"^[\d,.]")
SPLIT = re.compile(matrices.resolve(SPLIT_RULES, "split_pattern"))
