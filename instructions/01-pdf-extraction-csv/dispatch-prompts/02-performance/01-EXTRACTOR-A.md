...
- **Record output:** `ledgers/working/pdf-extraction-csv/02-performance/<file_id>/records-a.csv`
- **Coverage output:** `ledgers/working/pdf-extraction-csv/02-performance/<file_id>/coverage-a.csv`
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py claim --route 02-performance --agent A --model "<model name>"
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 02-performance --file <file_id> --agent A --through-page 3
```
...
EXTRACTION CHECKLIST, 22 required rules:
 1. Family follows the table, not the document type. One table, one
    family. This route may only use: performance_observation, fund_economics_observation, cash_flow_observation, definition_context.
