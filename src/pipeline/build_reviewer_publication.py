...
# The printed grouping stays as the page printed it, in the column its heading
# names. These columns say what that grouping is under one taxonomy. They are
# computed once, at the flatten stage, from context-grouping-map.csv, and they
# are part of
# fact_observation's own header by the time this stage reads it. This stage
# therefore carries them through with the rest of the row and names them here
# only to require their presence: appending them a second time produced a
# duplicate header and stopped reviewer publication outright.
...
def require_canonical_columns(observation_header: Sequence[str]) -> None:
    """Refuse a fact table that stopped carrying the canonical grouping.

    Dropping the second append means these columns now reach the reviewer file
    only through `fact_observation`'s header. Were the flatten to stop writing
    them, the reviewer file would lose five columns and still build, so the
    absence is checked instead of patched over here."""

    missing = [field for field in CANONICAL_REVIEW_FIELDS if field not in observation_header]
    if missing:
        raise ReviewerPublicationError(
            "fact_observation.csv lacks the canonical grouping columns: " + ", ".join(missing)
        )


# The printed industry and the four canonical readings a period carries from
# promotion and completion. The reviewer period file is built from the
# fund_periods header, so these travel with the rest of the row; they are named
# here to require their presence, because a reviewer file stating `Secondary
# Investments` with no reading beside it states a word and not a grouping.
...
def require_period_grouping_columns(period_header: Sequence[str]) -> None:
    """Refuse a fund_periods table that stopped carrying the taxonomy.

    The reviewer file inherits these columns from stage 80 rather than deriving
    a second taxonomy at publication. Were promotion to stop writing them, the
    reviewer file would lose five columns and still build, so the absence stops
    the stage here."""

    missing = [field for field in PERIOD_GROUPING_FIELDS if field not in period_header]
    if missing:
        raise ReviewerPublicationError(
            "fund_periods.csv lacks the printed grouping and its reading: "
            + ", ".join(missing)
        )
...
ANALYTICS_SUMMARY_FIELDS = (
    ...
    "metric_id",
    "group_name",
    # What the printed grouping in group_name reads as. Read from fund_master,
    # which carries the reading since F3; nothing is computed here and nothing
    # is computed in the dashboard, which had to head the column with the
    # vocabulary the word came from because this file stated no reading.
    "canonical_asset_class",
    "canonical_strategy",
    "canonical_sector",
    "row_count",
...
SUMMARY_RULES = "summary-statistics"
STATUS_ORDER = "status-precedence"
ORIGIN_VOCAB = "attribute-origin-vocabulary"
PROMOTION_VOCAB = "reviewer-promotion-status"
COVERAGE_RULES = "coverage-population"
PREFERENCE_ORDER = "record-preference-order"


def _percentile(values: list[float], percentile: float) -> float:
    if matrices.resolve(SUMMARY_RULES, "percentile_method") != "linear_interpolation":
        raise ReviewerPublicationError("summary-statistics.csv names an unknown method")
...
def quality_summary(rows: Iterable[Mapping[str, str]]) -> dict[str, str]:
    ...
    _order = tuple(
        row["output_value"]
        for row in sorted(matrices.load(STATUS_ORDER), key=lambda item: int(item["input_value"]))
    )
    if _order != ("FAIL", "PASS", "NOT_TESTABLE"):
        raise ReviewerPublicationError("status-precedence.csv names an unknown order")
    if counts["FAIL"]:
        status = "FAIL"
    elif counts["PASS"]:
        status = "PASS"
...
def attribute_review(
    ...
    for field in ATTRIBUTE_FIELDS:
        ...
        if printed:
            ...
            origin = next(
                row["input_value"]
                for row in matrices.load(ORIGIN_VOCAB)
                if row["output_value"] == "printed"
            )
            source = {
...
def build_observations() -> int:
    ...
    require_canonical_columns(observation_header)
    ...
    for observation in observations:
        ...
        if observation_id in promoted:
            promotion_status = matrices.resolve(PROMOTION_VOCAB, "promoted")
        elif observation_id in withheld:
            promotion_status = matrices.resolve(PROMOTION_VOCAB, "withheld")
        elif observation.get("subject_type") == "fund" and observation.get("subject_entity_id"):
            promotion_status = matrices.resolve(PROMOTION_VOCAB, "fund_context")
        else:
            promotion_status = matrices.resolve(PROMOTION_VOCAB, "other")
...
EXPOSURE_READING_FIELDS = ("canonical_asset_class", "canonical_strategy", "canonical_sector")


def _exposure_reading(
    group_name: str,
    allocation_rows: Sequence[Mapping[str, str]],
    master_by_fund: Mapping[str, Mapping[str, str]],
) -> dict[str, str]:
    """What the printed grouping of one exposure row reads as.

    The reading is the fund's own, taken from `fund_master`, and every fund
    grouped under one printed word reads the same way because the reading is a
    function of that word. A group whose funds disagree is named rather than
    reduced to one of them."""

    reading: dict[str, str] = {}
    for field in EXPOSURE_READING_FIELDS:
        values = {
            (master_by_fund.get(row.get("fund_id", ""), {}).get(field, "") or "")
            for row in allocation_rows
        }
        if len(values) > 1:
            raise ReviewerPublicationError(
                f"printed grouping {group_name!r} reads as more than one {field}: "
                + ", ".join(sorted(value or "<blank>" for value in values))
            )
        reading[field] = next(iter(values), "")
    return reading
...
def build_analytics_summary() -> int:
    ...
    coverage_specs = (
        ...
        (
            ...
            [
                ...
                if row.get("synthetic_parameter_set_id") == matrices.resolve(COVERAGE_RULES, "integrated_completion_set")
            ],
            integrated_metrics,
    ...
    master_by_fund = {row.get("fund_id", ""): row for row in masters}
        ...
        rows.append(
            {
                ...
                "metric_id": "target_weight",
                "group_name": strategy,
                **_exposure_reading(strategy, strategy_rows, master_by_fund),
...
def build_periods() -> int:
    ...
    require_period_grouping_columns(period_header)
    _, masters = read_csv(CSV_DIR / "fund_master.csv")
    ...
    for row in terms:
        ...
        _preferred = matrices.resolve(PREFERENCE_ORDER, "1", context="terms")
        if current is None or (
            row.get("provenance_type") == _preferred
            and current.get("provenance_type") != _preferred
        ):
    ...
    for period in periods:
        ...
        if matrices.resolve(PREFERENCE_ORDER, "holding_date", context="holdings") != "latest_on_or_before_period":
            raise ReviewerPublicationError("record-preference-order.csv names an unknown holding rule")
        holding_dates = [
            value
