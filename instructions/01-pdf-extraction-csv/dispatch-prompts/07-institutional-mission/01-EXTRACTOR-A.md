# Institutional Mission: EXTRACTOR A
...
- **Worklist:** `instructions/01-pdf-extraction-csv/worklists/active/07-institutional-mission.csv`
- **Record output:** `ledgers/working/pdf-extraction-csv/07-institutional-mission/<file_id>/records-a.csv`
- **Coverage output:** `ledgers/working/pdf-extraction-csv/07-institutional-mission/<file_id>/coverage-a.csv`
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py lane-check --route 07-institutional-mission --agent A
```
...
|---|---|---|---|---|
| `Foundations_Annual` | `SECONDARY` | `document_context` | one row per document | none |
| `Foundations_Annual` | `SECONDARY` | `financial_statement_observation` | one populated allowed source value cell | `beginning_capital`, `ending_capital`, `cash`, `total_assets`, `total_liabilities`, `net_assets`, `partners_capital`, `net_investment_income`, `investment_fair_value`, `investment_cost`, `fund_expense`, `interest_expense`, `realized_gain_loss`, `unrealized_gain_loss` |
| `Foundations_Annual` | `SECONDARY` | `position_observation` | one populated allowed source value cell for one named position | `quantity`, `cost`, `fair_value`, `market_value`, `notional`, `portfolio_weight`, `interest_rate`, `maturity_date` |
| `Foundations_Annual` | `SECONDARY` | `allocation_observation` | one populated allowed source value cell for one allocation bucket | `actual_allocation`, `target_allocation` |
| `Foundations_Annual` | `SECONDARY` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
| `Stewardship_Proxy_Report` | `SECONDARY` | `document_context` | one row per document | none |
| `Stewardship_Proxy_Report` | `SECONDARY` | `stewardship_observation` | one populated allowed source value cell | `meeting_count`, `vote_count`, `engagement_count`, `coverage`, `score`, `target` |
| `Stewardship_Proxy_Report` | `SECONDARY` | `stewardship_policy` | one separately headed operative policy statement | `stewardship_policy` |
| `Stewardship_Proxy_Report` | `SECONDARY` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
| `reporting` | Reporting duties. | `legal_clause` |
...
- **`metric_category` and `metric_name` are different columns and must not be swapped.** `metric_category` is a controlled value chosen from the list for that record family in **Allowed scope**. `metric_name` is the label printed on the page, verbatim. They are usually different text, and putting the printed label in `metric_category` is rejected.
  ...
  For a legal provision the same split applies: `term_category` is the controlled value and the printed wording goes in `text_raw`.
...
source_column_label, source_occurrence, metric_category, term_category
```
...
|---|---|---|
| Statement or note line items: totals, subtotals, capital-account movements, or investments **aggregated by asset class** instead of listed individually | `financial_statement_observation` | every column in that table |
...
| Engagement or proxy-voting activity reported as a count or proportion | `stewardship_observation` | every column in that table |
| A printed stewardship or voting policy statement | `stewardship_policy` | every column in that table |

### The one test that decides `position_observation` against `financial_statement_observation`
...
| Ending Market Value, Market Value, Net Asset Value, NAV, Remaining Value, Reported Value | `market_value` in a `position_observation` table |
| Fair Value, Estimated Fair Value | `fair_value` in `position_observation`; `investment_fair_value` in `financial_statement_observation` |
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py claim --route 07-institutional-mission --agent A --model "<model name>"
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 07-institutional-mission --file <file_id> --agent A
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 07-institutional-mission --file <file_id> --agent A --through-page 3
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py audit-file --route 07-institutional-mission --file <file_id> --agent A
```
...
EXTRACTION CHECKLIST, 22 required rules:
 1. Family follows the table. A named holding ->
