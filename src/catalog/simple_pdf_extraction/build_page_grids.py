...
from .page_grid import GRID_COLUMNS, grid_rows, inherit_headers, page_grid
...
TEXT_LAYER = "text-layer-classification"


def text_layer_of(txt_path: Path) -> str:
    """native, ocr, mixed or unknown, from the page-aligned TXT header."""
    if not txt_path.is_file():
        return matrices.resolve(TEXT_LAYER, "missing_file")
    head = txt_path.read_text(encoding="utf-8", errors="replace")[: int(matrices.resolve(TEXT_LAYER, "head_window"))]
    native = ocr = None
    for token, name in (
        (matrices.resolve(TEXT_LAYER, "native_token"), "native"),
        (matrices.resolve(TEXT_LAYER, "ocr_token"), "ocr"),
    ):
        if token in head:
            try:
