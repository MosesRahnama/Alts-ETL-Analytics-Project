# Institutional Report: ADJUDICATOR J1
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py lane-check --route 03-institutional-report --agent A
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 03-institutional-report --file <file_id> --agent A
python instructions/01-pdf-extraction-csv/workflow.py audit-file --route 03-institutional-report --file <file_id> --agent A
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py claim --route 03-institutional-report --agent J1 --model "<model name>"
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py compare --route 03-institutional-report --file <file_id>
```
...
| `Secondary_Pricing` | `CORE` | `document_context` | one row per document | none |
| `Secondary_Pricing` | `CORE` | `performance_observation` | one populated allowed source value cell | `return`, `irr`, `alpha`, `pme`, `direct_alpha`, `sharpe_ratio`, `tracking_error`, `yield`, `aum` |
| `Secondary_Pricing` | `CORE` | `fund_economics_observation` | one populated allowed source value cell | `commitment`, `paid_in_capital`, `paid_in_capital_multiple`, `contribution`, `distribution`, `nav`, `unfunded_commitment`, `recallable_distribution`, `tvpi`, `dpi`, `rvpi`, `moic`, `ownership_percentage`, `income`, `fee`, `carried_interest` |
| `Secondary_Pricing` | `CORE` | `position_observation` | one populated allowed source value cell for one named position | `quantity`, `cost`, `fair_value`, `market_value`, `notional`, `portfolio_weight`, `interest_rate`, `maturity_date` |
| `Secondary_Pricing` | `CORE` | `allocation_observation` | one populated allowed source value cell for one allocation bucket | `actual_allocation`, `target_allocation` |
| `Secondary_Pricing` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
- **Secondary_Pricing:** Blank, dash, N/A, calculated, inferred, template, and out-of-scope facts.

## Build and validate the final files

```powershell
python instructions/01-pdf-extraction-csv/workflow.py build-final --route 03-institutional-report --file <file_id>
python instructions/01-pdf-extraction-csv/workflow.py validate-final --route 03-institutional-report --file <file_id>
```
