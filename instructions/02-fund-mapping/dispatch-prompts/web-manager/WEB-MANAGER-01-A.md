# Web manager A: slice 01

*Generated by `name_normalization dispatch` from `02-WEB-MANAGER-A.md` and `manager-01-a.csv`. Regenerate this file; do not hand-edit it. Do not delete it when the slice is settled.*

- **Project root:** the repository root, the folder holding `README.md`; every path below is relative to it
- **Worksheet:** `data/normalization/worksheets/manager-01-a.csv`
- **Slice:** 24 GP lookups, covering 29 funds; 24 already finished in an earlier session. Every row is finished. This file stays: it is the standing brief for this slice.
- **Written cells:** `a_manager_name`, `a_source` in that file, and nothing else anywhere.
