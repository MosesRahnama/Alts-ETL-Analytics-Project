...
# cashflow-aggregation.csv, daycount-convention.csv, and solver-settings.csv
AGGREGATION = "cashflow-aggregation"
DAYCOUNT = "daycount-convention"
SOLVER = "solver-settings"

YEAR_DAYS = float(matrices.resolve(DAYCOUNT, "year_days"))
SOLVER_TOLERANCE = float(matrices.resolve(SOLVER, "tolerance"))
SOLVER_MAX_ITERATIONS = int(matrices.resolve(SOLVER, "max_iterations"))
...
def _normalize_cashflows(cashflows: Iterable[CashFlow]) -> list[CashFlow]:
    ...
    assert matrices.resolve(AGGREGATION, "same_day_flows") == "summed"
    by_date: dict[date, float] = defaultdict(float)
    refuse_non_finite = matrices.resolve(AGGREGATION, "non_finite_amount") == "refused"
    for cashflow_date, amount in cashflows:
        parsed_date = _as_date(cashflow_date)
        parsed_amount = float(amount)
        if refuse_non_finite and not isfinite(parsed_amount):
            raise ValueError("cash-flow amounts must be finite")
        by_date[parsed_date] += parsed_amount
    return sorted(by_date.items()) if matrices.resolve(AGGREGATION, "ordering") == "chronological" else list(by_date.items())
...
def xnpv(rate: float, cashflows: Iterable[CashFlow]) -> float:
    ...
    rate = float(rate)
    if rate <= float(matrices.resolve(DAYCOUNT, "minimum_rate")):
