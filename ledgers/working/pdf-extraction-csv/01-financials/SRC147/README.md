# SRC147 extraction evidence

Blind candidates, coverage, comparison, decisions, and final records for SRC147.

| File | Role |
|---|---|
| `records-a.csv` | Blind Extractor A observations. Header-only: coverage-final.csv carries the adjudicated status of every page. |
| `records-b.csv` | Blind Extractor B observations. Header-only: coverage-final.csv carries the adjudicated status of every page. |
| `coverage-a.csv` | Physical pages reviewed by Extractor A. |
| `coverage-b.csv` | Physical pages reviewed by Extractor B. |
| `pair-index.csv` | Paired agreements, disagreements, and one-sided candidate rows. Header-only: coverage-final.csv carries the adjudicated status of every page. |
| `coverage-diff.csv` | Page-level differences between the two extraction lanes. |
| `resolution.csv` | Adjudicator decisions for candidate pairs. Header-only: coverage-final.csv carries the adjudicated status of every page. |
| `coverage-resolution.csv` | Adjudicator decisions for page-coverage differences. |
| `records-final.csv` | Source-backed final observations for one document. Header-only: coverage-final.csv carries the adjudicated status of every page. |
