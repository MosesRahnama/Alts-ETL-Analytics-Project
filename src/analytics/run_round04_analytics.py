...
# What a measured rate means: the fee basis and the method of the period it was
# measured on. run_extracted_analytics writes the same two columns on the
# extracted-only table, so the two fund_metrics files carry one header.
QUALIFIER_RESULT_COLUMNS = ("input_fee_basis", "input_method")

FUND_METRIC_COLUMNS = (*ANALYSIS_RESULT_COLUMNS, *QUALIFIER_RESULT_COLUMNS)

# benchmark_selection says why this row carries the benchmark it carries:
# `mapped` when the fund's canonical strategy named it, otherwise the
# strategy-benchmark-map.csv default row that fired.
PME_RESULT_COLUMNS = (
    *ANALYSIS_RESULT_COLUMNS,
    "benchmark_selection",
    *QUALIFIER_RESULT_COLUMNS,
)

# The taxonomy reading that travels beside every printed grouping the release
# publishes. A printed word with no reading beside it is a grouping the
# analytics join on and no reader can resolve.
GROUPING_COLUMNS = (
    "sector",
...
QUALITY_GATE = "analytics-quality-gate"
QUANTUM_RULE = "weight-quantum"
JOIN_POLICY = "benchmark-join-policy"
# How far the terminal index level may sit behind the as-of date before the
# comparison stops being as-of at all.
MAXIMUM_TERMINAL_LAG_DAYS = int(
    matrices.resolve(JOIN_POLICY, "maximum_terminal_lag_days")
)
APPROVAL_POLICY = "quality-approval-policy"
INDEX_RULES = "benchmark-index-construction"
STATUS_POLICY = "record-status-policy"
ROUNDING = "rounding-policy"
FIELD_ORDER = "field-fallback-order"
ID_RULES = "identifier-composition"
...
# The label a period whose page states no fee basis or method carries. It is a
# statement, not a gap: measurability-rule.csv admits it and requires the word.
UNSTATED = "unstated"

# Which printed column each grouping is read under, and where its reading lands.
GROUPING_READ_CONTEXTS = ("strategy", "classifier_output")


QUALITY_POPULATION = matrices.resolve(QUALITY_GATE, "quality_population")
WEIGHT_QUANTUM = Decimal(matrices.resolve(QUANTUM_RULE, "weight_quantum"))
...
class _BenchmarkSeries:
    ...
    def as_of(self, target_date: date) -> _BenchmarkPoint:
        # benchmark-join-policy.csv: the latest observation on or before the date.
        position = bisect_right(self.dates, target_date) - 1
        if position < 0 and matrices.resolve(JOIN_POLICY, "before_first_observation") == "refused":
            raise AnalyticsError(
                f"benchmark has no observation on or before {target_date.isoformat()}"
            )
        return self.points[position]

    def typical_spacing_days(self) -> int:
        """The series' own cadence, so the tolerance below is not a guess.

        A daily series may legitimately have no observation on the as-of date
        itself; an annual one may be eleven months short of it. Reading the
        cadence off the series covers both without a table of periodicities.
        """
        if len(self.dates) < 2:
            return 0
        gaps = sorted(
    ...
    def refuse_if_past_end(self, target_date: date, benchmark_id: str) -> None:
        """Refuse an as-of date the series stops short of.

        The join is backward-only, so a date past the last observation quietly
        takes the last level the series holds and reports it as the level on
        the as-of date. One run priced every PME at a period end 25 days after
        the series ended, and no output said so. A date inside the series is a
        different matter: taking Friday's level for a Sunday cash flow is the
        join working as intended, so only the end of the series is bounded.
        """
        spacing = self.typical_spacing_days()
        if spacing == 0:
            # One observation states no cadence, so there is nothing to measure
            # the gap against. The other benchmark checks still apply.
            return
        last = self.dates[-1]
        beyond_end = (target_date - last).days
        tolerance = max(MAXIMUM_TERMINAL_LAG_DAYS, spacing)
        if beyond_end > tolerance:
            raise AnalyticsError(
                f"benchmark {benchmark_id} ends {last.isoformat()}, "
                f"{beyond_end} days before the as-of date {target_date.isoformat()}; "
                f"benchmark-join-policy.csv allows {tolerance}. Extend the series "
                "through the as-of date, or move the as-of date to a date the "
                "series covers."
            )
...
def grouping_reading(row: Mapping[str, str]) -> dict[str, str]:
    """What a row's printed groupings read as under context-grouping-map.csv.

    The period already carries the reading promotion and completion wrote, and
    that is what is used when it is there. Where a period predates those
    columns, the printed words are read through the same matrix here, so the
    allocation states a taxonomy and never a bare printed word. A printed word
    the matrix carries no row for leaves the reading blank."""

    carried = {name: _text(row.get(name)) for name in GROUPING_COLUMNS}
    if any(carried[name] for name in GROUPING_COLUMNS[1:]):
        return carried
    printed = _text(row.get("strategy"))
    for context in GROUPING_READ_CONTEXTS:
        if not printed:
            break
        match = next(
            (
                item
                for item in matrices.rows_in(GROUPING_MAP, context)
                if item["input_value"] == printed
            ),
            None,
        )
        if match is None:
            continue
        carried["canonical_asset_class"] = matrices.decode(match["output_value"])
        carried["canonical_strategy"] = match["canonical_strategy"]
        carried["canonical_sub_strategy"] = match["canonical_sub_strategy"]
        break
    if carried["sector"] and not carried["canonical_sector"]:
        match = next(
            (
                item
                for item in matrices.rows_in(GROUPING_MAP, "sector")
                if item["input_value"] == carried["sector"]
            ),
            None,
        )
        carried["canonical_sector"] = match["sector"] if match else ""
    return carried


def stated_qualifiers(period: Mapping[str, str]) -> tuple[str, str]:
    """The fee basis and method a period states for the rates it carries.

    measurability-rule.csv admits a page that states neither and requires the
    word `unstated`, so the reader meets a labelled rate instead of a bare
    number. The money-weighted columns are read first because the rates the
    analytics recompute are money-weighted."""

    if matrices.resolve(
        MEASURABILITY, "unstated_fee_basis", context="fee_basis"
    ) != "measurable_labelled":
        raise AnalyticsError(f"{MEASURABILITY}: unsupported unstated_fee_basis policy")
    basis = _first_text(
        period.get("reported_irr_fee_basis"), period.get("period_return_fee_basis")
    )
    method = _first_text(
        period.get("reported_irr_method"), period.get("period_return_method")
    )
    return basis or UNSTATED, method or UNSTATED


def funds_with_one_basis(
    periods: Sequence[Mapping[str, str]],
) -> tuple[set[str], int]:
    """The funds whose own periods agree on a fee basis, and how many were not.

    A fund stating net on one period and gross on another has two series, and a
    multiple or a rate built across both compares two different measures.
    measurability-rule.csv refuses that fund and has the count reported, so the
    refusal is a number a reader sees rather than a silent drop."""

    if matrices.resolve(
        MEASURABILITY, "mixed_fee_basis_within_fund", context="fee_basis"
    ) != "refused_and_counted":
        raise AnalyticsError(f"{MEASURABILITY}: unsupported mixed_fee_basis_within_fund policy")
    stated: dict[str, set[str]] = defaultdict(set)
    every_fund = set()
    for period in periods:
        fund_id = _text(period.get("fund_id"))
        every_fund.add(fund_id)
        basis, _method = stated_qualifiers(period)
        if basis != UNSTATED:
            stated[fund_id].add(basis)
    mixed = {fund_id for fund_id, bases in stated.items() if len(bases) > 1}
    return every_fund - mixed, len(mixed)


def benchmark_for_fund(
    canonical_strategy: str,
    first_cashflow: date | None,
    series_start: Mapping[str, date],
    default_benchmark_id: str,
) -> tuple[str, str]:
    """The benchmark one fund is priced against, and why that one.

    strategy-benchmark-map.csv keys the choice on the fund's canonical strategy,
    and `default_benchmark_id` is what config/integrated_completion.yml
    configures. Three conditions send a fund to that default, and the reason
    travels on the row so a reader sees which fired rather than meeting one
    index on every fund with nothing said about it: the fund states no strategy
    inside its class, the mapped proxy has no promoted series, or its series
    begins after the fund's first cash flow and prices no observation there."""

    if not canonical_strategy:
        return default_benchmark_id, "no_canonical_strategy"
    identifier = matrices.resolve(
        BENCHMARK_IDENTIFIERS, "benchmark_format", context="benchmark"
    )
    mapped = identifier.format(
        ticker=matrices.resolve(
            BENCHMARK_MAP, canonical_strategy, context="canonical_strategy"
        )
    )
    start = series_start.get(mapped)
    if start is None:
        return default_benchmark_id, "mapped_series_absent"
    if first_cashflow is not None and start > first_cashflow:
        return default_benchmark_id, "mapped_series_starts_after_first_cashflow"
...
def calculate_fund_metrics(
    ...
    *,
    ...
    for period in sorted(selected_periods, key=_period_sort_key):
        ...
        input_fee_basis, input_method = stated_qualifiers(period)
        first_of_period = len(output)
        ...
        try:
            ...
            for row in output[first_of_period:]:
                row["input_fee_basis"] = input_fee_basis
        ...
        # Every row this period produced names the basis and the method of the
        # period behind it, including the rows written before the XIRR ran.
        for row in output[first_of_period:]:
            row["input_fee_basis"] = input_fee_basis
            row["input_method"] = input_method
    if not output:
...
def calculate_pme_results(
    ...
    *,
    ...
    through_date: date | None = None,
    strict_quality: bool = True,
    canonical_strategy_by_fund: Mapping[str, str] | None = None,
) -> list[dict[str, str]]:
    """Calculate Kaplan-Schoar PME and Direct Alpha with backward-only joins.

    `canonical_strategy_by_fund` keys the comparison: a fund is priced against
    the benchmark strategy-benchmark-map.csv names for its canonical strategy,
    and `benchmark_id` is the default the map's own default rows point at. With
    no mapping given every fund takes that default, which is the state this
    argument exists to end."""
    if not benchmark_id.strip():
        raise AnalyticsError("benchmark_id is required")
    series_cache: dict[str, _BenchmarkSeries] = {}

    def series_for(name: str) -> _BenchmarkSeries:
        if name not in series_cache:
            series_cache[name] = _build_benchmark_series(
                benchmark_returns, name, periodicity
            )
        return series_cache[name]

    # The first date each promoted series covers. A fund whose first cash flow
    # predates its mapped proxy has no observation to discount that flow at, so
    # the map's default row prices it and the row says which condition fired.
    series_start: dict[str, date] = {}
    for row in benchmark_returns:
        if not _active(row):
            continue
        name = _text(row.get("benchmark_id"))
        moment = _date(row.get("return_date"), "benchmark_returns.return_date")
        if name and (name not in series_start or moment < series_start[name]):
            series_start[name] = moment
    first_cashflow: dict[str, date] = {}
    for row in fund_cashflows:
        if not _active(row):
            continue
        moment = _date(row.get("cashflow_date"), "fund_cashflows.cashflow_date")
        fund = _text(row.get("fund_id"))
        if fund not in first_cashflow or moment < first_cashflow[fund]:
            first_cashflow[fund] = moment
    selected_periods = [
        row
        for row in fund_periods
        if _active(row)
    ...
    for period in sorted(selected_periods, key=_period_sort_key):
        ...
        provenance_type = _analysis_provenance(period)
        input_fee_basis, input_method = stated_qualifiers(period)
        if canonical_strategy_by_fund is None:
            fund_benchmark_id, benchmark_selection = benchmark_id, ""
        else:
            fund_benchmark_id, benchmark_selection = benchmark_for_fund(
                _text(canonical_strategy_by_fund.get(fund_id, "")),
                first_cashflow.get(fund_id),
                series_start,
                benchmark_id,
            )
        series = series_for(fund_benchmark_id)
        nav = _nonnegative_number(period.get("nav"), "nav")
        dated_cashflows, cashflow_ids = _dated_cashflows(
            cashflows_by_position.get(_position_key(period, "fund_periods"), ()),
            period_date,
        ...
        series.refuse_if_past_end(period_date, fund_benchmark_id)
        ...
        for row in (
            ...
            row["benchmark_selection"] = benchmark_selection
            row["input_fee_basis"] = input_fee_basis
            row["input_method"] = input_method
            output.append(row)
    if not output:
...
def bounded_equal_weights(
    ...
    if residual and matrices.resolve(ROUNDING, "residual_assignment") == "sorted_fund_order_within_bounds":
        ...
        output.append(
            {
                ...
                "expected_return": _first_text(
                    *(
                        row.get(rank_row["output_value"])
                        for rank_row in sorted(
                            matrices.rows_in(FIELD_ORDER, "portfolio_positions.expected_return"),
                ...
                "expected_volatility": "",
                "liquidity_score": "",
                "provenance_type": matrices.resolve(STATUS_POLICY, "position_provenance"),
                "source_document_id": "",
                "synthetic_parameter_set_id": _text(row.get("synthetic_parameter_set_id")),
                "optimization_run_id": matrices.resolve(ID_RULES, "run_id_format", context="analytics.run").format(
                    optimization_run_id=optimization_run_id, quality_run_id=quality_state.run_id
                ),
                "record_status": matrices.resolve(STATUS_POLICY, "position_status"),
                # The taxonomy reading travels with the printed word it reads,
                # so the allocation states a grouping a reader can resolve. It
                # sits last because the published header is a fixed order and
                # these columns joined it after the rest.
                **grouping_reading(row),
            }
        )
    return output
    ...
    *,
    ...
    fund_master = read_csv_rows(
        ...
        required_columns=(
            ...
            "fund_name",
            "provenance_type",
            # PME keys on this column. Requiring it here means a fund_master
            # that stopped carrying the taxonomy stops the run instead of
            # quietly pricing every fund against the configured default.
            "canonical_strategy",
        ),
    )
    ...
    # One fund stating net on some periods and gross on others is two series,
    # and a multiple or a rate built across both compares two measures.
    # measurability-rule.csv refuses that fund; the count is returned so the
    # caller prints it.
    one_basis, refused_funds = funds_with_one_basis(periods)
    periods = [row for row in periods if _text(row.get("fund_id")) in one_basis]
    if not periods:
        raise AnalyticsError(
            "every fund mixes a net and a gross fee basis inside its own series"
        )
    canonical_strategy_by_fund = {
        _text(row.get("fund_id")): _text(row.get("canonical_strategy"))
    ...
    pme = calculate_pme_results(
        ...
        canonical_strategy_by_fund=canonical_strategy_by_fund,
    )
    ...
    write_csv_rows(output_root / "fund_metrics.csv", FUND_METRIC_COLUMNS, metrics)
    write_csv_rows(output_root / "pme_results.csv", PME_RESULT_COLUMNS, pme)
    write_csv_rows(
    ...
    return {
        ...
        "funds_refused_for_mixed_fee_basis": refused_funds,
        "benchmarks_mapped": sum(1 for row in pme if row["benchmark_selection"] == "mapped"),
    }
    ...
    for run_id, run_rows in by_run.items():
        ...
        for (record_id, rule_id), row in indexed.items():
            ...
            if status not in {"PASS", "FAIL", "SKIP"}:
                ...
                break
            if (
                expected_rules[rule_id] == matrices.resolve(APPROVAL_POLICY, "blocking_severity")
                and status == matrices.resolve(APPROVAL_POLICY, "blocking_status")
            ):
...
def _entity_id(row: Mapping[str, object]) -> str:
    ...
    if (
        matrices.resolve(ID_RULES, "fund_only_when", context="analytics.entity") == "no_position_parts"
        and not any((lp_id, lp_name, share_class_name))
    ):
        return fund_id
    return _stable_id(
        "POSITION",
...
def _dated_cashflows(
    ...
    for row in rows:
        ...
        if cashflow_date > terminal_date:
            continue
        # field-fallback-order.csv (analytics.cashflow_amount): base outranks raw.
...
def _build_benchmark_series(
    ...
    level = float(matrices.resolve(INDEX_RULES, "base_level"))
    for row in selected:
...
def _analysis_provenance(row: Mapping[str, str]) -> str:
    ...
    if value not in set(matrices.mapping(PROVENANCE_RULES)):
...
def _active(row: Mapping[str, str]) -> bool:
    ...
    return status in {
        matrices.decode(piece)
        for piece in matrices.resolve(STATUS_POLICY, "active_statuses").split("|")
    }
...
def _format_float(value: float) -> str:
    return format(value, matrices.resolve(ROUNDING, "float_format")).rstrip("0").rstrip(".") or "0"
...
def _stable_id(*parts: str) -> str:
    separator = matrices.resolve(ID_RULES, "id_separator", context="analytics.stable_id")
    assert matrices.resolve(ID_RULES, "id_sanitize", context="analytics.stable_id") == "non_alphanumeric_to_underscore"
    assert matrices.resolve(ID_RULES, "id_case", context="analytics.stable_id") == "upper"
    return separator.join(
        re.sub(r"[^A-Za-z0-9]+", separator, part).strip(separator).upper() for part in parts
    )
