...
| `demo/` | A two-fund walkthrough with one clean record and one planted-error record. |
| `documents/` | Local PDF sources plus page text, 300 DPI pictures required for extraction, and document grids. |
| `extracted/` | Published source observations, page coverage, route consolidations, and relational tables. |
| `integrated/` | Gap, cell-origin, reconciliation, benchmark-policy, and defect evidence for same-fund completion. |
...
| `schemas/` | Document routing, record families, metric and term vocabularies, and the family surveys that define the field list. |
