# Fund Legal Documents: EXTRACTOR A
...
- **Record output:** `ledgers/working/pdf-extraction-csv/05-fund-legal-docs/<file_id>/records-a.csv`
- **Coverage output:** `ledgers/working/pdf-extraction-csv/05-fund-legal-docs/<file_id>/coverage-a.csv`
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py lane-check --route 05-fund-legal-docs --agent A
```
...
| `PPM` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
| `LPA` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
| `Subscription` | `SECONDARY` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
| `Side_Letter` | `CORE` | `definition_context` | one printed footnote, definition, methodology note, or legend entry | none |
...
|---|---|---|
| A printed economic or governance term: fee rate, carry, preferred return, term length, key person | `legal_term` | every column in that table |
...
| A numbered or separately headed operative provision | `legal_clause` | every column in that table |
| A subscription's printed commitment, entity type, jurisdiction, or execution detail | `subscription_reference` | every column in that table |
| A printed quantitative answer to a due-diligence question: firm or fund AUM, counts, leverage, liquidity | `ddq_quantitative_observation` | every column in that table |
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py claim --route 05-fund-legal-docs --agent A --model "<model name>"
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 05-fund-legal-docs --file <file_id> --agent A --through-page 3
```
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py audit-file --route 05-fund-legal-docs --file <file_id> --agent A
```
...
EXTRACTION CHECKLIST, 22 required rules:
 1. Family follows the table, not the document type. One table, one
    family. This route may only use: legal_term, definition_context, legal_clause, subscription_reference, ddq_quantitative_observation, fund_economics_observation.
