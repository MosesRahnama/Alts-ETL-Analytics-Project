...
SCOPE_POLICY = "fund-attribute-scope-policy"
STATUS_POLICY = "attribute-status-policy"
VINTAGE_RULES = "vintage-parse"
CANON_RULES = "attribute-canon-rules"
SPELLING_RULES = "attribute-spelling-rules"
EVIDENCE_ORDER = "evidence-preference-order"

FIELDS = tuple(
    row["output_value"]
    for row in sorted(matrices.rows_in(STATUS_POLICY, "field"), key=lambda item: int(item["input_value"]))
)
STAMP_FIELDS = tuple(row["input_value"] for row in matrices.rows_in(STATUS_POLICY, "stamp_field"))
SETTLED = {row["input_value"] for row in matrices.rows_in(STATUS_POLICY, "settled_status")}
FUND_PREFIX = matrices.resolve(STATUS_POLICY, "fund_prefix")
assert FIELDS == ("vintage_year", "strategy", "asset_class", "geography")
assert STAMP_FIELDS == ("vintage_year", "strategy")
assert SETTLED == {"unique", "unique_canonical", "decided"}
assert FUND_PREFIX == "FUND_"
...
def canon_vintage(value: str) -> str:
    match = re.search(matrices.resolve(VINTAGE_RULES, "vintage_pattern"), value)
    if match:
        return match.group(0)
    if matrices.resolve(VINTAGE_RULES, "no_match") != "stripped_text":
        raise ValueError("vintage-parse.csv names an unknown fallback")
    return value.strip()


def canon_label(value: str) -> str:
    # attribute-canon-rules.csv, applied in rank order.
    rules = tuple(
        row["output_value"]
        for row in sorted(matrices.load(CANON_RULES), key=lambda item: int(item["input_value"]))
    )
    if rules != ("casefold", "dash_to_space", "non_alphanumeric_to_space",
                 "collapse_whitespace", "drop_investments", "added_to_add"):
        raise ValueError("attribute-canon-rules.csv names an unknown sequence")
...
def pick_spelling(counter: Counter[str]) -> str:
    ...
    order = tuple(
        row["output_value"]
        for row in sorted(matrices.load(SPELLING_RULES), key=lambda item: int(item["input_value"]))
    )
    if order != ("most_often_printed", "longest", "alphabetical"):
        raise ValueError("attribute-spelling-rules.csv names an unknown order")
...
def classify(field: str, values: Counter[str]) -> tuple[str, str, str]:
    ...
    if not values:
        return "", matrices.resolve(STATUS_POLICY, "no_values", context="classify"), ""
    if len(values) == 1:
        return next(iter(values)), matrices.resolve(STATUS_POLICY, "single_value", context="classify"), variants
...
def attribute_scope_allowed(row: dict[str, str]) -> bool:
    """Only whole-fund context supplies fund-constant attributes."""
    return matrices.resolve_or_star(SCOPE_POLICY, row.get("value_scope", "")) == "include"
...
def harvest() -> int:
    ...
    for row in observations:
        ...
        for field in FIELDS:
            value = (row.get(field) or "").strip() if attribute_scope_allowed(row) else ""
            if value:
...
def attribute_evidence_lookup(
    ...
    for row in observations:
        fund_id = row.get("subject_entity_id", "")
        if row.get("subject_type") != "fund" or fund_id not in settled or not attribute_scope_allowed(row):
            continue
    ...
    for fund_id, attrs in sorted(settled.items()):
        for field, chosen in sorted(attrs.items()):
            found = sorted(
                ...
                # evidence-preference-order.csv governs this listing order.
                key=lambda row, _order=tuple(
                    r["output_value"]
                    for r in sorted(matrices.rows_in(EVIDENCE_ORDER, ""), key=lambda item: int(item["input_value"]))
                ): (
                    row.get(_order[0], ""),
                    int(row.get(_order[1]) or 0),
                    row.get(_order[2], ""),
                ),
            )
            if not found:
...
def write_inherit_log(lookup: dict[str, dict[str, str]] | None = None) -> int:
    ...
    for row in read_csv(FACT_OBSERVATION):
        fund_id = row.get("subject_entity_id", "")
        if row.get("subject_type") != "fund" or fund_id not in lookup or not attribute_scope_allowed(row):
            continue
