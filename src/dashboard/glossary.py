...
TABLE_NOTES: dict[str, str] = {
    ...
    "data/extracted/review/document-summary.csv": (
        "One row per reviewed PDF. It counts the fields each LLM typed, "
    ...
    "data/csv/pme_results.csv": (
        ...
        "multiple and Direct Alpha, each against the benchmark named on that result. Row detail lists every "
    ...
    "data/csv/synthetic_parameters.csv": (
        ...
        "PDF-reported values. ASSUMED parameters are generation settings used to make SYNTHETIC "
        "cells; ASSUMED is empty of a row label on fund_master."
    ),
    ...
    "data/integrated/gap-ledger.csv": (
        ...
        "after, and how the fill was decided. resolution_type is EXTRACTED, DERIVED, IMPUTED, "
        "or SYNTHETIC."
    ),
    "data/integrated/reconciliation-results.csv": (
        ...
        "row confirms the fill left the printed value as it was. A fail here is a pipeline break, "
        "separate from a fund-table FAIL on a money rule."
    ),
    "data/integrated/detection-scorecard.csv": (
        "One row per deliberate test-error type, showing how many errors were inserted into "
        "defect-periods.csv and how many the intended rule detected. Those twelve errors stay "
        "out of the data/csv fund tables."
    ),
    ...
    "docs/FINAL-RELEASE-AUDIT.csv": (
        "Source collection, document preparation, independent extraction, and adjudication precede "
        "automated publication. Each row names an output and its check. Consecutive row numbers "
        "describe the complete process; internal execution IDs remain in row details. PASS records "
        "the named check at its recorded time, not a new review of every printed value. Original "
        "proposals remain unchanged; final validation checks the reviewed evidence. Quality checks "
        "can finish successfully while their result tables retain source exceptions."
    ),
    "ledgers/pipeline/transformation-receipts.csv": (
        "One row per output of a data-changing command, recording its stage, inputs, output, row count, "
        "result, and any error. stage_id names the producing stage. Earlier failures remain recorded; "
        "release-checks.csv records the latest executed check for each stage. Checksums remain in row details."
    ),
    "docs/PROJECT-MANIFEST.csv": (
        "One row per tracked file or folder, showing whether it belongs in the public repository, "
        "which folder guide owns it, and the role it serves. TRACK ships with the repository. "
        "LOCAL_ONLY stays on the machine. Source PDFs are public; page pictures are local. Size "
        "and checksum remain in row detail."
    ),
    ...
    "docs/CSV-LINEAGE.csv": (
        "One row per CSV in the repository. It names the prior CSV, the module that wrote the file, "
        "and any agent operation and instruction file used to create its rows."
    ),
    "src/catalog/simple_pdf_extraction/field_guide.py": (
        "One definition for each of the 47 columns every extractor writes. The dashboard reads the "
...
DATABASE_TABLE_NOTES: dict[str, str] = {
    ...
    "benchmark_returns": "One dated return per benchmark series. Each row records its benchmark, return basis, and use restrictions.",
...
COLUMN_NOTES: dict[str, str] = {
    ...
    "provenance_type": "Origin of the record. EXTRACTED: printed on a cited page. DERIVED: copied inside a fund from a printed value. IMPUTED: no page states the field, so a declared rule supplied a default; imputation_method names the rule. SYNTHETIC: made from the stated settings. SYNTHETIC periods sit on real FUND_ IDs. On fund_master the label is the origin of the fund's identity, not of every cell in the row: a fund a page names carries EXTRACTED while its strategy or vintage may be filled, and the cell-lineage table gives each field its own origin.",
    ...
    "record_status": "ACTIVE rows take part in the analytics. Other stored marks stay in the review files.",
    "created_at": "When the row was written.",
    "formula_id": "The named calculation on the Analytics cards: DPI, RVPI, TVPI, XIRR, KS-PME, or Direct Alpha.",
    ...
    "imputation_method": "The declared rule that supplied a fund-model field value the reviewed PDFs leave blank. SEEDED_RANGE_DRAW_V1 spreads a hash of the run seed and fund ID across the configured 2000 to 2024 range, and sets every vintage_year no page prints. CONFIGURED_STRATEGY_FALLBACK_V1 writes the configured strategy and the canonical asset class it maps to. NAME_KEYWORD_V1 reads a keyword from the fund's own name into name_hint. Each rule states a default and cites no page.",
    ...
    # positions and dates
    "fund_period_id": "Identifier of one performance snapshot for one fund (or LP position) on one date. Two rows share this ID only when they are the same snapshot.",
    ...
    "perspective": "Whose figure this is: fund_total, lp_position, or share_class. Keep perspectives in separate totals.",
    ...
    "dpi": "Distributions to paid-in: cash returned per unit of cash drawn. 1.00x is dollar-for-dollar cash back on that slice.",
    "rvpi": "Residual value to paid-in: NAV per unit of cash drawn. 1.00x is dollar-for-dollar remaining value on that slice.",
    "tvpi": "Total value to paid-in: distributions plus NAV, per unit of cash drawn. 1.00x is dollar-for-dollar total value on that slice.",
    ...
    "quality_population": "The quality-run ID plus PASS that licensed the period into analytics.",
    ...
    "sector": "The industry of the subject, as the page prints it under an industry heading; blank where the page groups by asset class or style instead.",
    ...
    "synthetic_parameters.adjudication_status": "Whether a person has reviewed and approved this generation parameter for use.",
    ...
    "cell-lineage.source_table": "The fund-model table a DERIVED value was copied from.",
    ...
    "canonical_doc_type": "The document's listed type, picked by two LLMs each working alone and an audit.",
    ...
    "metric_category": "What the value measures, from the one vocabulary. Example: metric_name Total Value to Paid In maps to metric_category tvpi.",
    "term_category": "For a clause: what kind of provision it is, from the term vocabulary.",
    "metric_name": "The printed label for the measure, in the page's own words. The shared name sits in metric_category.",
    ...
    "asset_class": "The asset-class grouping printed on the page, verbatim: a private-capital class such as Real Estate, a plan sleeve such as Fixed Income, or a statement grouping. An industry heading goes to sector and a benchmark name stays the row's own label, never here.",
    "horizon": "The measurement period of a return figure, such as 1-Yr or since inception. Rows with different horizons stay in separate totals.",
    ...
    "value_kind": "The printed form of the cell: number, currency, percent, multiple, text, or none. A figure under a dollar heading can print as a number; the currency column then holds USD.",
    "value_sign": "The sign the page gives the value, including parentheses for negatives. Rule R01 flags a negative balance.",
    ...
    "pair_id": "Identifier of the A/B pair the row was picked from.",
    "pair_status": "How the two typed rows compared. agreement: same value. VALUE: 1.21 against 1.22. CLASSIFICATION: TVPI against IRR for the same number. CONTEXT: same number under a different date, subject, or unit. one-sided: one LLM found the field.",
    ...
    "resolution_decision": "MERGE joins two readings that match the page. ACCEPT_A or ACCEPT_B keeps one LLM. ADD types a missed field. REJECT drops a row empty of a printed fact.",
    ...
    "definition_keys": "On a definition row, the printed marker the note is keyed by; on a value row, the printed markers citing the definitions that govern it, pipe-joined.",
    "method": "How a return was computed, from the page's own statement; unstated where the page states none.",
    "fee_basis": "The fee treatment the page states for the value: net, gross, partial, or unstated.",
    "value_scope": "The grain the page states for the value, such as fund_position, plan_total, or statement_line; unstated where the page states none.",
    "canonical_measure_id": "The listed measure a semantic-map rule assigned; blank until a rule matches.",
    "mapping_rule_id": "The semantic-map rule that assigned the listed measure; blank until a rule matches.",
    "canonical_asset_class": "The asset class under one taxonomy, read off the printed asset_class or strategy grouping: one of the six private-capital classes, a public sleeve, an umbrella such as real_assets, or a statement grouping. Blank where the page prints no class.",
    "categories": "Every field name this printed label was mapped to across the corpus, separated by a space and a pipe. More than one means the label alone does not say which measure it is.",
    "record_families": "The kinds of printed table this label appeared in, separated by a space and a pipe.",
    "example_observation_id": "One evidence row carrying this label, to open as a starting point.",
    "is_monetary": "Whether the row states an amount of money, read from the currency first and then the printed form. A figure under a dollar heading prints as a number and is still money.",
    "canonical_sector": "The industry under one taxonomy, read off the printed sector column; blank where the page groups by class or style instead of industry.",
    "canonical_sub_strategy": "The sub-strategy under one taxonomy: the finer label inside a strategy, such as early-stage inside venture capital, read off the printed strategy or sub_strategy grouping. Blank where the page prints only the broad strategy or none.",
    "method_qualifier": "The method the page states for the value on this row, held under its own name because the wide tables also carry a measure column named method. Same values as the method column of the evidence file: time-weighted, money-weighted, Modified Dietz, annualized, actuarial, or unstated.",
    "qualifier_note": "What the row's method, fee basis, or scope reads and where it came from, carried beside the value so a reader sees the qualifier without opening the evidence row.",
    "reported_irr_fee_basis": "The fee treatment stated for the reported IRR on this row: net, gross, partial, or unstated. Promotion refuses an IRR whose evidence row states no basis, so a value here is always the basis the page carried.",
    "reported_irr_method": "The method stated for the reported IRR on this row, such as money-weighted or actuarial, or unstated where the page states none.",
    "period_return_fee_basis": "The fee treatment stated for the period return on this row: net, gross, partial, or unstated. A return reaches this column only with a stated method and fee basis; the rest stay in fund_observations.",
    "period_return_method": "The method stated for the period return on this row, such as time-weighted, Modified Dietz, or annualized.",
    "input_fee_basis": "The fee treatment of the rates this result was calculated from, carried up from the period rows it read. A fund whose own series mixes net and gross is refused and counted rather than measured.",
    "input_method": "The method of the rates this result was calculated from, carried up from the period rows it read, so a result never blends a time-weighted rate with a money-weighted one without saying so.",
    "benchmark_selection": "Why this row's benchmark was chosen: mapped where the fund's canonical strategy has a row in the benchmark map, default where it has none and the configured default was used.",
    "name_hint": "The strategy keyword the fund's own name contains. A hint about the name, empty of evidence about the fund, and kept out of strategy and sub_strategy.",
    "canonical_strategy": "One strategy name every printed, inferred, and benchmark vocabulary maps into, so the tables can be joined on one key.",
    "definition_text": "The printed footnote, definition, methodology note, or legend entry, verbatim.",
    "applies_to": "What the printed note governs, where the page states it: a column, a table, or the rows carrying its marker.",
    "qualified": "yes where the category's number means one thing only with a stated method, fee basis, or scope.",
    "required_dimensions": "The dimensions a qualified category must state, pipe-joined.",
    "disposition": "Where values of this category go: promoted to the fund model, evidence_only, or qualitative.",
    ...
    "pairs": "One comparison row per PDF field proposed by either extractor, including fields proposed by only one LLM.",
    "pair_rows": "One comparison row per PDF field proposed by either extractor, including fields proposed by only one LLM.",
    "found_by_both": "PDF fields both LLMs typed.",
    ...
    "wave": "The intake batch the file arrived in. Empty of a document type.",
    ...
    "analysis_tier": "PME_CORE may go into the comparison; ADVANCED_DAILY and MARKET_CONTEXT are market background.",
    ...
    "replacement_chars": "Glyphs the PDF encodes in a font with no Unicode mapping, so the text layer holds U+FFFD where the page prints a character. Ten corpus files carry them, and in each the lost glyphs are table-of-contents dot leaders and ornaments rather than words or figures.",
    ...
    "documents_unreadable": "Documents the LLM could not open.",
    ...
    "extraction-runs.run_id": "Identifier of one timed extraction run: the reading group and the document it read.",
    ...
    "quality_results.status": "PASS: the recomputed value agrees within the allowed gap. FAIL: the figures break the money rule. SKIP: the rule lacked an input, so it stayed out of PASS and FAIL.",
    ...
    "FINAL-RELEASE-AUDIT.order": "Consecutive position in the full process, including work before the automated release command.",
    "FINAL-RELEASE-AUDIT.phase": "Preparation, extraction, review, automated publication, or closing verification.",
    "FINAL-RELEASE-AUDIT.stage": "The activity performed at this process step.",
    "FINAL-RELEASE-AUDIT.output": "The files or recorded check result produced by this activity.",
    "FINAL-RELEASE-AUDIT.check_description": "The scope of the check recorded for this activity; proposal retention and final source validation are separate checks.",
    "FINAL-RELEASE-AUDIT.execution_order": "Internal execution ID used by the release script; gaps reserve positions for inserted stages. Preparation and extraction have no execution ID.",
    "FINAL-RELEASE-AUDIT.status": "The most recent recorded result for the named check: PASS or FAIL. NOT_RUN means no result was recorded; STALE means the recorded command differs from the current command. This is a dated result, not continuous certification.",
    "FINAL-RELEASE-AUDIT.checked_at_utc": "UTC time when the named check completed or failed.",
    "FINAL-RELEASE-AUDIT.evidence": "The release-check ledger and the exact check identifier supporting this result.",
    "FINAL-RELEASE-AUDIT.result_detail": "The recorded check summary or error message.",
    ...
    "Printed grouping": "The grouping word the source file prints for this row, whichever vocabulary it belongs to: an asset class, a strategy, a real-asset sector, or a vehicle type. context-grouping-map.csv says which taxonomy each word reads into.",
    ...
    "Run": "One timed extraction run of one document by one reading group.",
    ...
    "Cost per document": "Estimated cost for one reading, averaged over the documents of this type.",
    "Total cost": "Estimated cost for one reading over all documents of this type.",
    "Predicted turns": "Turns the two-coefficient law asks for, from this run's pages and rows.",
    "Billed turns": "Turns the run actually took, counted from the session log.",
    "Predicted cost": "Predicted turns priced at the measured rate per turn.",
    "Billed cost": "What the provider charged for the run.",
    "Predicted over billed": "Predicted turns divided by billed turns. Above one means the law overcharges this shape.",
    ...
    "Imputed": "Fund-model field values supplied by the recorded imputation method, such as a configured default or seeded estimate.",
...
# Whole families of reviewer columns are one idea applied to several fields:
# every settled attribute carries its own value, where it came from, and the
# row, document, page, and quote behind it. Naming the pattern once keeps the
# definitions honest as fields are added, which forty near-identical entries
# would not.
COLUMN_SUFFIX_NOTES: tuple[tuple[str, str], ...] = (
    ("_source_observation_id", "The evidence row this settled value was taken from."),
    ("_source_document_id", "The document the evidence row for this settled value came from."),
    ("_source_page", "The physical page that printed the evidence for this settled value."),
    ("_source_quote", "The printed line the evidence for this settled value was read from."),
    ("_origin", "How this value was settled: printed on the page, inherited from the same fund, or missing."),
    ("_ids", "The identifiers of the rows this row was built from, separated by a space and a pipe."),
    ("_rules", "The named checks this row failed, separated by a space and a pipe."),
)
COLUMN_PREFIX_NOTES: tuple[tuple[str, str], ...] = (
    ("effective_", "The value in force for this row after the settled fund attribute is applied; the printed column beside it keeps the page's own words."),
    ("canonical_", "The listed-type reading of the printed grouping under one taxonomy, from context-grouping-map.csv; the printed column beside it keeps the page's own words."),
    ("term_clause_", "A field of the fund-term clause this row is joined to."),
    ("term_", "A field of the fund terms this row is joined to."),
    ("holding_", "A field of the holdings this row is joined to."),
    ("portfolio_", "A field of the portfolio allocation this row is joined to."),
    ("lineage_", "How this row was settled between the two reading groups: the pair, its status, the rows behind it, and the decision."),
    ("recomputed_", "The measure recalculated from the fund tables at build, printed beside the reported figure so the two can be compared."),
    ("analytics_", "A field describing how the analytical result for this row was produced."),
    ("quality_", "The outcome of the quality checks that read this row."),
)
...
def column_note(column: str, table: str = "") -> str:
    ...
    for suffix, note in COLUMN_SUFFIX_NOTES:
        if lowered.endswith(suffix):
            return note
    for prefix, note in COLUMN_PREFIX_NOTES:
