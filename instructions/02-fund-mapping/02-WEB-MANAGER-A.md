# Web Manager A: name the GP of each standardized fund
...
>
> Fill only `a_manager_name` and `a_source`. Those are the only two
> columns the file has. Agent B works a separate file that stays closed here, and
...
different managers, leave the name blank and say so in `a_source` after
`WEB_MANAGER: `, so the adjudicator can split the row.

Web search only. This brief is for Agent A. The slice is one part of a round split across
...
flowchart TD
    ...
    P -- "GP's own site" --> W["a_manager_name = firm as printed<br/>a_source = WEB_MANAGER: URL | one sentence"]:::done
    ...
    N --> S
    P -- "nothing says it" --> U["a_manager_name blank<br/>a_source = WEB_MANAGER: unresolved"]:::todo
...
| Column | Action |
|---|---|
| `a_manager_name` | The GP, adviser, or management company of **that fund**, as a public source names it |
| `a_source` | Prefix `WEB_MANAGER: ` then the URL, a pipe, and one short published sentence |
...
On search failure leave `a_manager_name` blank and write `WEB_MANAGER: unresolved` in `a_source`. Do not write `unresolved` into the name cell.
...
Every row in the slice has `a_manager_name` filled or deliberately blank, and `a_source` filled or marked `WEB_MANAGER: unresolved`. Report in at most six lines: names filled, unresolved, rows that were not funds, anything left unguessed.
