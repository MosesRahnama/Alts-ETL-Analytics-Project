...
# The manifest's exclusion list, so a tool cache never gets asked for a folder
# guide. No cycle: the manifest imports nothing from here.
from src.repository.build_project_manifest import EXCLUDED_DIRECTORIES, skip_local_tree

PROJECT_ROOT = Path(__file__).resolve().parents[2]
SKIP_DIRS = set(EXCLUDED_DIRECTORIES)
...
COUNT_RULES = "guide-counts"
EMPTY_RULES = "empty-table-reasons"

if matrices.resolve(GATES, "hand_written_guides") != "listed_in_HAND_WRITTEN":
    raise RuntimeError("pipeline-gates.csv names an unknown guide policy")
HAND_WRITTEN = frozenset({
    ...
    "src/pipeline",
    "GP-Scoring",
    "GP-Scoring/01-inputs",
    "GP-Scoring/02-scoring",
    "GP-Scoring/03-report",
    "GP-Scoring/04-diagnostics",
    "GP-Scoring/05-scenarios",
    "GP-Scoring/06-report",
    "GP-Scoring/data",
    "GP-Scoring/data/documents",
    "GP-Scoring/research",
    "GP-Scoring/tests",
    "GP-Scoring/archive",
    "GP-Scoring/v2-archive",
    "One-Day-Pricing",
    "One-Day-Pricing/01-inputs",
    "One-Day-Pricing/02-pricing",
    "One-Day-Pricing/03-report",
    "One-Day-Pricing/archive",
    "One-Day-Pricing/tests",
    "RAG",
    "RAG/src",
    "RAG/src/alts_rag",
    "RAG/config",
    "RAG/tests",
    "RAG/tests/fixtures",
    "RAG/tests/fixtures/corpus",
    "RAG/tests/fixtures/corpus/data",
    "RAG/tests/fixtures/corpus/data/documents",
    "RAG/tests/fixtures/corpus/data/documents/txt",
    "RAG/tests/fixtures/corpus/data/documents/grids",
    "RAG/tests/fixtures/corpus/data/documents/pdf",
    "RAG/tests/fixtures/corpus/data/schemas",
    "RAG/tests/fixtures/corpus/data-gathering",
    "RAG/evaluation",
    ".github",
    ".github/workflows",
})
...
NEXT_READMES = {
    ...
    "data/normalization/transformations": ("../../../ledgers/pipeline/README.md", "Matrix checks and migration evidence"),
...
FOLDER_ORDER: dict[str, tuple[str, ...]] = {
    ...
    "src/dashboard": (
        "build_dashboard.py", "build_rag_dashboard.py", "build_field_guide.py", "page.py",
        "release_explanations.py", "field_guide_template.html",
    ),
...
PURPOSES = {
    ...
    "output": "Local browser-review artifacts generated during dashboard inspection; these files do not feed the published data release.",
    "tmp": "Scratch space for agents and operators. No pipeline stage, gate, or generator reads this folder, and nothing temporary belongs at the repository root.",
    "output/playwright": "Local browser-test captures and reports from dashboard inspection; these files do not feed the published data release.",
    ...
    "data/documents/pdf": "The 452 public and FOIA source PDFs listed in data-gathering/source_ledger.csv.",
    ...
    "data/documents/grids": "Coordinate-derived table-cell grids for all routed source documents.",
    ...
    "data/integrated": "Gap, cell-origin, reconciliation, benchmark-policy, and defect evidence for same-fund completion.",
    ...
    "data/normalization/transformations": "Rule-family decision matrices for 420 inventoried data-decision sites. Rows name the input, output, context, decision owner, and former code location; src/common/matrices.py reads them and stage 20 checks them. Four non-data sites have structure dispositions in ledgers/pipeline/migration-dispositions.csv.",
    ...
    "data/schemas": "Document routing, the record families, the vocabulary of metric and term names, and the family surveys that define the field list.",
    ...
    "Expansion/Data/02-discovery": "Expansion candidate queue copied from the validated scrape list. Not the canonical source ledger.",
    "Expansion/Data/03-acquisition": "Native-source download receipts with SHA-256, bytes, and magic checks.",
    "Expansion/Data/04-admission": "PASS, HOLD, and REJECT surface for Expansion sources before any canonical merge.",
    "Expansion/Data/05-handoff": "Schema-discovery and PDF-materialization worklist. No production extraction rows.",
    "Expansion/Data/source-documents": "Native Expansion bytes held outside data/documents/pdf until admission.",
    "Expansion/Data/source-documents/market-reports": "Advisor secondary-market reports kept as native files.",
    "Expansion/Data/source-documents/ilpa": "ILPA templates and guidance kept as native files.",
    "Expansion/Data/source-documents/sec-filings": "SEC EDGAR HTML, XML, and related native filings.",
    ...
    "instructions/01-pdf-extraction-csv": "Independent extraction and source review retain printed values, definitions, table context, and page evidence; shared candidate and final checks reject omissions and contradictory interpretations.",
    ...
    "instructions/01-pdf-extraction-csv/worklists/active": "Current extraction assignments, including completed documents and pending work.",
    ...
    "ledgers/working/pdf-extraction-csv": "Independent A/B records, comparisons, review decisions, and finals, one folder per assigned document.",
    ...
    "src/catalog/simple_pdf_extraction": "The active 47-column field list, prompt builder, validator, and publisher.",
    "src/common": "Shared financial mathematics and decision-matrix access.",
    "audit/python-reorg": "Current implementation evidence plus historical inventories, plans, and append-only records for the decision-matrix migration.",
    "src/dashboard": "The main dashboard, RAG reviewer page, and local field guide rendered from published artifacts.",
...
TITLES = {
    ...
    "instructions/01-pdf-extraction-csv": "PDF extraction and source review",
...
PYTHON_ROLES = {
    ...
    "matrices.py": "Validate and read decision matrices, separate shared contexts, reject absent lookups, and write the stage-20 check record.",
    ...
    "build_rag_dashboard.py": "Read the checked-in RAG evidence export and write the standalone reviewer page rag.html.",
    "build_field_guide.py": "Write the local-only reviewer field guide inside the ignored audit tree from the current dashboard payload and the maintained explanatory shell.",
    "page.py": "The dashboard shell: style, browser code, and the block kinds the builder emits.",
    "release_explanations.py": "The explanation of each release-audit step, shown under the dashboard's audit table and in the field guide.",
    ...
    "test_migration_gate.py": "The migration check on a scratch repository, including all-site evidence, structure dispositions, allowed paths, and prior-tag comparison.",
    ...
    "test_generator.py": "Generation checks, a clean universe with no quality failures, and every injected defect detected.",
    ...
    "test_transformation_matrices.py": "Matrix validation, lookups, contexts, compatibility assertions, and the stage-20 check record.",
    ...
    "test_project_structure.py": "Every folder guide is current, the manifest avoids recursive reviewer-page hashes, and the live structure validates.",
    ...
    "test_promotion_to_fund_level.py": "Every promoted row names a fund and an accepted document, money reaches one scale, and the promotion check refuses a missing document.",
...
FILE_ROLES = {
    ...
    "IMPLEMENTATION-REPORT.md": "Completed changes, checker findings, validation results, and remaining operator actions.",
    "IMPLEMENTATION-REPORT.csv": "One verification row per implemented milestone or checker closeout.",
    "ROADMAP.md": "Milestones, acceptance checks, current status, and append-only execution record.",
    "master-transformation-inventory.md": "Historical three-pass inventory of 424 coded decision sites, followed by the current implementation status.",
    "NORMALIZATION-MATRIX-DISPATCH.md": "Closed inventory assignment retained as historical evidence.",
    "PIPELINE-CHANGE-REPORT.md": "Current migration mechanics, checker corrections, validation, and version-control boundary.",
    "python-file-io-map.csv": "One row per Python module with the files it reads and writes.",
    "transformation-inventory.csv": "Agent A inventory of coded conversions, classifications, filters, defaults, and parsing rules.",
    "transformation-inventory-b.csv": "Agent B inventory of coded conversions, classifications, filters, defaults, and parsing rules.",
    "transformation-inventory-c.csv": "Agent C inventory of coded conversions, classifications, filters, defaults, and parsing rules.",
    "transformation-inventory.md": "Agent A coverage log, findings, target matrices, and migration order.",
    "transformation-inventory-b.md": "Agent B coverage log, findings, target matrices, and migration order.",
    "transformation-inventory-c.md": "Agent C coverage log, findings, target matrices, and migration order.",
    "transformation-migration-plan.md": "Superseded first design and its implemented replacement.",
    ...
    "pdf-wide-records.csv": "Published 47-column source observations.",
    ...
    "records-a.csv": "Extractor A source records.",
    "records-b.csv": "Extractor B source records.",
    ...
    "fund_holdings.csv": "Compatibility projection; extracted position authority is fund_position.csv.",
    "investment_owner.csv": "Economic and reporting owners used by normalized positions, with reviewed identity and optional fund links.",
    "investment_target.csv": "Normalized investee identities, keeping funds, companies, unresolved securities, and other targets distinct.",
    "investment_instrument.csv": "Normalized fund interests, equity, debt, derivatives, and other instruments issued by one target.",
    "fund_position.csv": "Owner-by-instrument positions with source holding IDs, dates, values, and distinct ownership and portfolio-weight fields.",
    "lookthrough_edge.csv": "Source-supported look-through relationships only; an empty file means no relationship was inferred.",
    "holding_field_lineage.csv": "Field-level origins for normalized owners, targets, instruments, positions, and look-through records.",
    "normalized-holdings-refusals.csv": "Source holding rows kept out of normalized positions with an explicit refusal reason.",
    "holding-owner-map.csv": "Reviewed decisions that identify the economic or reporting owner for holdings-bearing documents.",
    ...
    # data/warehouse
    "extracted.duckdb": "The document-level star schema from data/extracted/tables: published documents, entities, observations, and their source references.",
    ...
    "document_type_field_schema.csv": "One row per document type of the pre-ratification survey taxonomy (18) stating typical grain, default perspective, fund-name rule, provided fields, and downstream use; the ratified 20-value list is data-gathering/document-types.csv.",
    ...
    "CSV-TEMPLATE.csv": "The record columns, in order: the header every records-a, records-b, and records-final file must carry.",
    "COVERAGE-TEMPLATE.csv": "The 15 coverage columns: the header every coverage-a, coverage-b, and coverage-final file must carry.",
    "RESOLUTION-TEMPLATE.csv": "Pair identifier, decision and reason followed by the complete reviewed record columns.",
    ...
    "EXTRACTION-CATEGORY-COVERAGE.md": "All 17 report categories, completed file IDs, and pending extraction assignments.",
    "EXTRACTION-CATEGORY-COVERAGE.csv": "Published and active documents with category, source paths, assignment folder, and published page count.",
    ...
    "FINAL-RELEASE-AUDIT.csv": "Complete process sequence with recorded check results and evidence references.",
    "RELEASE-STEPS.csv": "Authored process descriptions; automated stage membership and order are checked against the release command.",
    "release-checks.csv": "Append-only results of executed preparation, publication, and repository checks.",
    ...
    "rag.html": "The RAG reviewer page as last built: architecture, evidence controls, evaluation, use cases, and the local evidence console.",
    "architecture.html": "Static visual description of the RAG stores, evidence block types, query operations, and return envelope.",
    "start-local.ps1": "Start the local RAG dashboard with one reviewer session over the reviewed documents.",
    "requirements-lock.txt": "Pinned local-vector package versions used by the tested RAG runtime.",
    "summary.json": "Current retrieval totals and the held-out hybrid-activation decision.",
    "field_guide_template.html": "The field-guide shell, controls, and definitions; the builder inserts current dashboard data, including the release-step explanations.",
...
def project_directories() -> list[Path]:
    """The folders a released tree carries a guide for.

    Tool caches and gitignored operator trees are skipped by the same two
    rules the manifest uses. `audit/` is local working memory that Git never
    publishes, and only `audit/diagnostics` was named here, so the generator
    demanded a generated folder guide inside `audit/explanations` and reported
    the hand-written one there as drift."""
    ...
    directories.extend(
        ...
        and not skip_local_tree(relative(path))
    )
...
def purpose_for(rel: str) -> str:
    ...
    if rel.startswith("data/documents/images/") and len(parts) == 4:
        return (
            "300 DPI images of each physical PDF page use page-NNN.png numbering; "
            "`data-gathering/src/render_image_corpus.py` generates these local files."
        )
    if rel.startswith("ledgers/working/pdf-extraction-csv/"):
        if parts[-1].startswith("SRC"):
            return f"Independent extraction records, page coverage, and review outputs for {parts[-1]}."
...
EMPTY_REASONS = {
    ...
    "ledgers/working/pdf-extraction-csv/": (
        "Header-only: coverage-final.csv records the reviewed page statuses."
    ),
}


def empty_reason(rel: str) -> str:
    if matrices.resolve(EMPTY_RULES, "header_only_csv") != "explained":
        raise RuntimeError("empty-table-reasons.csv names an unknown policy")
    if rel.startswith("ledgers/working/pdf-extraction-csv/"):
        if not (PROJECT_ROOT / rel).with_name("coverage-final.csv").is_file():
            return "Header-only: extraction and page review are pending."
...
def _describe_file(path: Path) -> str:
    ...
    if rel.startswith("data/normalization/transformations/") and suffix == ".csv":
        return (
            "Decision rows for one rule family: each row names the input, output, context where needed, "
            "decision owner, and former code location."
        )
    ...
    if rel.startswith("data/extracted/raw/") and suffix == ".csv":
        return "Every adjudicated records-final.csv of this route concatenated in document order; the issued contract columns, nothing altered."
...
def bulk_rows(directory: Path, files: list[Path]) -> list[tuple[str, str]]:
    ...
    for suffix, count in sorted(counts.items()):
        ...
        if suffix == ".csv" and matrices.resolve(COUNT_RULES, "csv_suffix") == "excluded_from_type_table":
            continue
        role = {
...
def diagram_for(rel: str, files: list[Path], subdirs: list[Path]) -> str:
    if rel.startswith("ledgers/working/pdf-extraction-csv/") and rel.split("/")[-1].startswith("SRC"):
        if not any(path.name == "records-final.csv" for path in files):
            return ""
        return """```mermaid
sequenceDiagram
...
def ignored_files() -> frozenset[str]:
    """Files Git ignores, by path from the root.

    A folder guide lists what a clone of the repository carries. The page
    renders are ignored (large, rebuilt by render_image_corpus.py), so the 41
    image-folder guides that listed them read as drift in every clone, where
    the folders hold the guide alone.
    """
...
def render_readme(directory: Path) -> str:
    ...
    files = [
        ...
        and relative(path) not in ignored
    ]
    subdirs = sorted(
