...
# The star loader shares the fund-model loader's null policy.
NULLSTR = matrices.resolve("database-null-policy", "nullstr")
assert matrices.resolve("load-null-policy", "empty_cell_means") == "null"
        ...
        for table, path in sources:
            ...
            connection.execute(
                f'''INSERT INTO "{table}" ({columns})
                    SELECT {columns}
                    FROM read_csv('{escaped}', header = true, all_varchar = true, nullstr = '{NULLSTR}')'''
            )
