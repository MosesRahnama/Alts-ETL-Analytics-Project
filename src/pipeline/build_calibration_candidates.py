...
STRATEGY_SCOPE = "calibration-strategy-scope"
METRIC_MAP = "calibration-metric-map"
CLEANING = "calibration-input-cleaning"
SERIALIZATION = "numeric-serialization"
PANEL_IDENTITY = "calibration-source-panel"
PANEL_GATES = "calibration-panel-gates"
FORMULAS = "calibration-formulas"
OUTPUT_LABELS = "calibration-output-labels"
CANDIDATE_STATUS = "calibration-candidate-status"

PARAMETER_SET_ID = matrices.resolve(PANEL, "parameter_set_id")
SOURCE_DOCUMENT_ID = matrices.resolve(PANEL, "source_document")
SOURCE_PAGE = matrices.resolve(PANEL, "source_page")
SOURCE_TABLE = matrices.resolve(PANEL, "source_table")
SOURCE_STRATEGIES = set(matrices.mapping(STRATEGY_SCOPE))
EXPECTED_FUND_COUNT = int(matrices.resolve(PANEL, "expected_fund_count"))
METRICS = matrices.mapping(METRIC_MAP)
assert PARAMETER_SET_ID == "SOURCE_CALIBRATION_REAL_ESTATE_V1"
assert SOURCE_STRATEGIES == {"Core", "Opportunistic", "Value Add"}
assert EXPECTED_FUND_COUNT == 33
...
def _read_rows(path: Path) -> list[dict[str, str]]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if matrices.resolve(CLEANING, "cell_whitespace") != "stripped":
            raise CalibrationCandidateError("calibration-input-cleaning.csv names an unknown rule")
        return [
...
def _plain_number(value: float) -> str:
    if matrices.resolve(SERIALIZATION, "plain_number_format", context="calibration") != ".10f stripped":
        raise CalibrationCandidateError("numeric-serialization.csv names an unknown plain format")
...
def _source_panel(
    ...
    selected = [
        ...
        and row.get("subject_type") == matrices.resolve(PANEL, "subject_type")
    ...
    if matrices.resolve(PANEL_GATES, "duplicate_observation_id") != "refused":
        raise CalibrationCandidateError("calibration-panel-gates.csv names an unknown rule")
    ...
    if matrices.resolve(PANEL_IDENTITY, "panel_identity") != "one_lp_schedule":
        raise CalibrationCandidateError("calibration-source-panel.csv names an unknown identity")
...
def build_candidates(
    ...
    for metric_id, short_name in METRICS.items():
        ...
        formulas = (
            (f"{short_name}_mean", statistics.fmean(values), matrices.resolve(FORMULAS, "mean")),
            (f"{short_name}_sd", statistics.stdev(values), matrices.resolve(FORMULAS, "sd")),
        )
        for parameter_name, value, formula_id in formulas:
            candidates.append(
                {
                    "parameter_id": (
                        matrices.resolve(FORMULAS, "id_prefix") + parameter_name.upper()
                    ),
                    "parameter_set_id": PARAMETER_SET_ID,
                    "strategy": matrices.resolve(OUTPUT_LABELS, "strategy"),
                    "sub_strategy": "",
                    "parameter_name": parameter_name,
                    "value_numeric": _plain_number(value),
                    "value_text": "",
                    "unit": matrices.resolve(OUTPUT_LABELS, "unit"),
                    "provenance_type": matrices.resolve(OUTPUT_LABELS, "provenance_type"),
                    ...
                    "adjudication_status": matrices.resolve(CANDIDATE_STATUS, "adjudication_status"),
                    "active": matrices.resolve(CANDIDATE_STATUS, "active"),
                }
            )
