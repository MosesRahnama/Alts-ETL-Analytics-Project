...
from .build_project_manifest import (
    EXCLUDED_DIRECTORIES,
    OUTPUT,
    entries,
    gitignored_root_names,
    sha256,
)
...
# Bytecode and tool caches are not listed: every Python invocation, this check
# included, creates them, so forbidding them made `python -m pytest` fail on
# its own footprint, and `ruff` on its own .ruff_cache. The manifest carries
# the one list, read from release-expectations.csv, and this gate skips exactly
# what the manifest skips.
SKIP_NAMES = set(EXCLUDED_DIRECTORIES)
...
# with nowhere to put a working file, and they used the repository root
# instead: one round left two helper scripts, a verifier, and a header copy
# beside README.md. A `tmp` folder anywhere else is still debris.
SCRATCH_ROOT = "tmp"
...
def check_root_index(errors: list[str]) -> None:
    ...
    if not readme.is_file():
        return
    text = readme.read_text(encoding="utf-8-sig", errors="replace")
    skipped = SKIP_NAMES | gitignored_root_names()
