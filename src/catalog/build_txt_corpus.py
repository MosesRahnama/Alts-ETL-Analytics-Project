...
# `replacement_chars` counts the glyphs the PDF encodes in a font with no
# Unicode mapping, which arrive as U+FFFD. `write_txt` has always counted them
# into the file's own header and then dropped the number from the manifest row,
# so the one place a dispatcher looks said nothing: ten corpus files carry
# between 1 and 46,339 of them and reading 442 file headers was the only way to
# find out. The number now reaches the manifest, and `replacement_char_report`
# backfills it from the headers of text already on disk.
MANIFEST_HEADER = [
    "file_id", "filename", "txt_filename", "doc_type", "pages",
    "native_pages", "ocr_pages", "empty_pages", "chars", "replacement_chars",
    "seconds", "error",
]
REPLACEMENT_HEADER = "# replacement_chars_remaining:"
...
if matrices.resolve(SANITIZATION, "control_ranges") != "0x00-0x1F|0x7F|0x80-0x9F":
    raise RuntimeError("text-sanitization.csv names unknown control ranges")
CONTROL_CHARACTERS = {
    chr(code)
...
def _ocr_pdf_page(source: Path, page_index: int) -> str | None:
    ...
    if _OCR_ENGINE is None:
        _OCR_ENGINE = RapidOCR(
            print_verbose=False,
            text_score=float(matrices.resolve(OCR_SETTINGS, "text_score")),
            use_angle_cls=matrices.resolve(OCR_SETTINGS, "use_angle_cls") == "true",
            use_text_det=matrices.resolve(OCR_SETTINGS, "use_text_det") == "true",
            min_height=int(matrices.resolve(OCR_SETTINGS, "min_height")),
            width_height_ratio=int(matrices.resolve(OCR_SETTINGS, "width_height_ratio")),
        )
        ...
        with pdfplumber.open(path) as pdf:
            ...
            for index in range(page_count):
                try:
                    ...
                    continue
                if squash(raw) and matrices.resolve(SANITIZATION, "native_page") == "kept":
...
def convert_one(source: dict[str, str], use_ocr: bool) -> dict[str, str]:
    ...
    return {
        ...
        "replacement_chars": str(counts["fffd"]),
...
def replacement_char_report() -> dict[str, int]:
    """Each text file's replacement-character count, read from its own header.

    A rebuild of the corpus is hours of work and would produce the same
    number: a glyph the PDF's font maps to nothing has no Unicode to recover,
    and the ten files carrying these are table-of-contents dot leaders and
    ornaments. The header written at extraction is the authority, so the count
    is read back from it rather than recomputed or re-extracted."""
    ...
    for path in sorted(TXT_DIR.glob("*.txt")):
        ...
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            for line in handle:
                ...
                elif line.startswith(REPLACEMENT_HEADER):
                    count = int(line.split(":", 1)[1].strip())
        if file_id:
            report[file_id] = count
    return report


def backfill_manifest_replacement_chars() -> int:
    """Write the header counts into the manifest without re-extracting."""

    if not MANIFEST_PATH.is_file():
        raise FileNotFoundError(MANIFEST_PATH)
    report = replacement_char_report()
    with MANIFEST_PATH.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    for row in rows:
        row["replacement_chars"] = str(report.get(row["file_id"], 0))
    with MANIFEST_PATH.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=MANIFEST_HEADER, lineterminator="\n")
        writer.writeheader()
        writer.writerows({field: row.get(field, "") for field in MANIFEST_HEADER} for row in rows)
    return sum(1 for row in rows if row["replacement_chars"] != "0")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--backfill-replacement-chars",
        action="store_true",
        help="write each file's replacement-character count into MANIFEST.csv from its header",
    )
    args = parser.parse_args(argv)

    if args.backfill_replacement_chars:
        affected = backfill_manifest_replacement_chars()
        print(f"PASS: MANIFEST.csv carries replacement_chars; {affected} files carry one or more")
        return 0
