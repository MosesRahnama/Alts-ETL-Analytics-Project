...
def test_review_outputs_tie_to_published_facts() -> None:
    ...
    assert {r["file_id"] for r in documents} == {r["document_id"] for r in review.read_rows(review.PROJECT_ROOT / "data/extracted/tables/dim_document.csv")}
    assert {r["document_id"] for r in facts} <= {r["file_id"] for r in documents}
