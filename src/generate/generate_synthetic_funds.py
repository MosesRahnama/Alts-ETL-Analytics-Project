...
# The fixture's parameters are rows in matrices under
STRATEGY_PARAMS = "fixture-strategy-parameters"
DEFECT_RULES = "defect-definitions"
BENCHMARK_MAP = "strategy-benchmark"
INCOME_YIELDS = "strategy-income-yield"
CURRENCY_WEIGHTS = "fund-currency-weights"
FX_RATES = "fx-to-usd"
VOLATILITY = "strategy-volatility"
LIQUIDITY = "strategy-liquidity"
FIXTURE_ROUNDING = "rounding-policy"
SCHEDULE_SHAPE = "fixture-schedule-shape"
POSITION_SHAPE = "fixture-position-shape"
OBSERVATION_MAP = "fixture-observation-map"
PORTFOLIO_SCHEMES = "fixture-portfolio-schemes"
...
DEFAULT_STRATEGIES: tuple[
    ...
    (
        row["input_value"], row["sub_strategy"], float(row["weight"]), float(row["size_mean"]),
        float(row["tvpi_terminal"]), float(row["tvpi_spread_a"]), float(row["tvpi_spread_b"]),
        float(row["tvpi_spread_c"]),
    )
    for row in matrices.load(STRATEGY_PARAMS)
)
...
EXPECTED_RULES = {
    ...
    for row in matrices.rows_in(DEFECT_RULES, "fixture")
}


STRATEGY_BENCHMARK = matrices.mapping(BENCHMARK_MAP)

# Annual income yield carried on NAV; strategies outside the map earn almost none.

STRATEGY_INCOME_YIELD = {
    row["input_value"]: float(row["output_value"])
    for row in matrices.load(INCOME_YIELDS)
    if row["input_value"] != "*"
}

# Reporting currency mix. A fund reports every fact in its own currency.

FUND_CURRENCY_WEIGHTS: tuple[tuple[str, float], ...] = tuple(
    (row["input_value"], float(row["output_value"])) for row in matrices.load(CURRENCY_WEIGHTS)
)

FX_TO_USD = {row["input_value"]: float(row["output_value"]) for row in matrices.load(FX_RATES)}
...
STRATEGY_VOLATILITY = {
    row["input_value"]: float(row["output_value"])
    for row in matrices.load(VOLATILITY)
    if row["input_value"] != "*"
}
...
STRATEGY_LIQUIDITY = matrices.mapping(LIQUIDITY)
STRATEGY_LIQUIDITY = {key: float(value) for key, value in STRATEGY_LIQUIDITY.items()}

LP_POSITION_SHARES = tuple(
    float(part) for part in matrices.resolve(POSITION_SHAPE, "lp_position_shares").split("|")
)
...
def _money(value: float) -> str:
    if matrices.resolve(FIXTURE_ROUNDING, "money_format", context="fixture") != ".2f with 1e-12 nudge":
        raise ValueError("rounding-policy.csv names an unknown fixture money format")
    return f"{round(value + 1e-12, 2):.2f}"


def _ratio(value: float) -> str:
    if matrices.resolve(FIXTURE_ROUNDING, "ratio_format", context="fixture") != ".6f":
        raise ValueError("rounding-policy.csv names an unknown fixture ratio format")
    return f"{value:.6f}"
...
def _call_weights(quarter_count: int, investment_quarters: int, rng: random.Random) -> list[float]:
    ...
    if matrices.resolve(SCHEDULE_SHAPE, "call_weight_decay") != "exp(-2.1*i/span)":
        raise ValueError("fixture-schedule-shape.csv names an unknown decay")
...
def _position_history(
    *,
    ...
    for index, when in enumerate(schedule.timeline):
        ...
        if matrices.resolve(SCHEDULE_SHAPE, "fee_base") != "commitment_then_beginning_nav":
            raise ValueError("fixture-schedule-shape.csv names an unknown fee base")
        ...
        _low, _high = (float(part) for part in matrices.resolve(SCHEDULE_SHAPE, "other_expenses_range").split(".."))
        other_expenses = round(
            max(beginning_nav, contributions, 1.0) * rng.uniform(_low, _high), 2
        )
...
def _position_specs(index: int, rng: random.Random) -> list[PositionSpec]:
    ...
    if index % int(matrices.resolve(POSITION_SHAPE, "lp_position_every")) == 0:
...
def _attach_observations(
    ...
    metric_map = {
        row["input_value"]: (row["output_value"], row["unit"], row["measure_kind"])
        for row in matrices.load(OBSERVATION_MAP)
    }
    _legacy_map = {
...
def _attach_allocations(
    ...
    if not periods:
        return
    counter = 0
    schemes = matrices.mapping(PORTFOLIO_SCHEMES)
    if dict(PORTFOLIO_DEFINITIONS) != schemes:
        raise ValueError("fixture-portfolio-schemes.csv and PORTFOLIO_DEFINITIONS disagree")
