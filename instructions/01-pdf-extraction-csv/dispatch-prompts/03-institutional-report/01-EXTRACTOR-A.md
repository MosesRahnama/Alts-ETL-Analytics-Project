...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 03-institutional-report --file <file_id> --agent A --through-page 1
```
...
RE-READ BEFORE THE NEXT DOCUMENT, the fifteen rules that decay:
 1. Family follows the table, not the document type. One table, one
    family. This route may only use: performance_observation, fund_economics_observation, position_observation, allocation_observation.
