...
def working_row(match: re.Match[str]) -> tuple[str, str, str, str]:
    ...
    if matrices.resolve("csv-lineage-rules", "unmatched_path") != "blank_origin":
        raise RuntimeError("csv-lineage-rules.csv names an unknown fallthrough")
    return "", WORKFLOW, "", ""
...
def resolve(rel: str) -> tuple[str, str, str, str]:
    ...
    if rel == "docs/FINAL-RELEASE-AUDIT.csv":
        return "docs/RELEASE-STEPS.csv|ledgers/pipeline/release-checks.csv", "src/repository/build_release_audit.py", "", ""
    if rel == "docs/RELEASE-STEPS.csv":
        return "", "", "Authored process descriptions checked against the release stage definitions", ""
    if rel == "ledgers/pipeline/release-checks.csv":
        return "", "src/pipeline/release_checks.py", "", ""
    if rel == "docs/CSV-LINEAGE.csv":
        return "", "src/repository/build_csv_lineage.py", "", ""
    if rel == "docs/EXTRACTION-CATEGORY-COVERAGE.csv":
        return (
