...
Field-list `2026-09-01.2`. Companion to `MASTER-EXTRACTION-SCHEMA.md`,
...
There are 47 fields on every row. Most are blank on any given
...
- `sector`: The printed industry or economic sector of the subject; it remains separate from the fund's asset class and strategy.
...
- `definition_keys`: On a definition_context row, the printed marker the note is keyed by; on a value row, the pipe-joined printed markers citing the definitions that govern it.
- `method`: How a return was computed, from the page's own statement: time_weighted, money_weighted, modified_dietz, simple_period, annualized, actuarial, or unstated.
- `fee_basis`: The fee treatment the page states for the value: net, gross, partial, or unstated.
- `value_scope`: The grain the page states for a value: fund_position, single_holding, portfolio_total, plan_total, statement_line, allocation_bucket, or unstated.
...
- `metric_category`: any name in the metric vocabulary of `EXTRACTION-METRIC-CATEGORIES.csv`,
  90 names, each with a definition and unit hint; `term_category`: any of its
