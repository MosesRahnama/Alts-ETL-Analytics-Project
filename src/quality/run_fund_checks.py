...
# The rule catalogue, the precision fields, the sign conventions, and the rest
# of the decisions below are rows in matrices under
# data/normalization/transformations, read here and checked by release stage 20.
# stays a code fallback because config/quality_rules.yml already overrides it.
RULE_SEVERITY = "quality-rule-severity"
PRECISION_FIELD_MAP = "printed-precision-fields"
PRECISION_POLICY = "printed-precision-policy"
POSITION_KEY = "position-key"
SIGN_POLICY = "cashflow-sign-policy"
SKIP_POLICY = "quality-skip-policy"
XIRR_EXCLUSIONS = "xirr-excluded-cashflow-types"
...
def _rules_from(rule_set: str) -> tuple[tuple[str, str], ...]:
    return tuple(
        ...
        for row in matrices.load(RULE_SEVERITY)
        if row["rule_set"] == rule_set
    )


RULES: tuple[tuple[str, str], ...] = _rules_from("core")

EXTENDED_RULES: tuple[tuple[str, str], ...] = _rules_from("extended")

DEFAULT_TOLERANCES = {
    name: float(value)
    for name, value in matrices.mapping(DEFAULT_TOLERANCE_POLICY).items()
}

# The vocabulary name a printed cell carries in fact_observation, mapped to
# the fund_periods column the promotion wrote it into; the rows of
# printed-precision-fields.csv. The precision of the printed cell is what
# bounds how far a recomputation can drift from the printed ratio.
PRECISION_FIELDS = matrices.mapping(PRECISION_FIELD_MAP)
XIRR_EXCLUDED_CASHFLOW_TYPES = matrices.members(
    XIRR_EXCLUSIONS, "excluded_from_xirr"
)
...
def printed_half_unit(value_raw: str, multiplier: float = 1.0) -> float | None:
    ...
    fraction = float(matrices.resolve(PRECISION_POLICY, "last_digit_fraction"))
    blank = float(matrices.resolve(PRECISION_POLICY, "blank_multiplier"))
    return fraction * (10.0 ** -decimals) * float(multiplier or blank)
...
def printed_precision_from_observations(
    ...
    for period in fund_periods:
        ...
        for observation_id in (item.strip() for item in ids):
            ...
            if field is None:
                continue
            slack_source = matrices.resolve(SKIP_POLICY, "rounding_slack_source")
            if slack_source != "printed_half_unit":
                raise matrices.MatrixError(
                    f"{SKIP_POLICY}: unsupported rounding_slack_source {slack_source!r}"
                )
...
def run_quality_checks(
    ...
    *,
    ...
    source_fund_master: Sequence[Mapping[str, str]] | None = None,
) -> list[dict[str, str]]:
    ...
    source_masters = {row["fund_id"]: row for row in source_fund_master or []}
    ...
    for row_number, period in enumerate(fund_periods, start=1):
        ...
        evaluations = _evaluate_period(
            ...
            (source_masters.get(fund_id) if source_fund_master is not None
             and period.get("provenance_type") == "EXTRACTED" else masters_by_fund.get(fund_id)),
...
def _position_key(row: Mapping[str, str]) -> tuple[str, str, str, str]:
    ...
    fields = tuple(row_["output_value"] for row_ in sorted(
        matrices.load(POSITION_KEY), key=lambda item: int(item["input_value"])
    ))
    lp_id = _text(row.get(fields[1]))
    if fields[2] != "<lp_name_when_no_lp_id>":
        raise matrices.MatrixError(f"{POSITION_KEY}: unsupported LP-name policy {fields[2]!r}")
    return (
        _text(row.get(fields[0])),
        lp_id,
        "" if lp_id else _text(row.get("lp_name")),
        _text(row.get(fields[3])),
    )
...
def _evaluate_cashflow_signs(
    ...
    signs = matrices.mapping(SIGN_POLICY)
    negative_types = {name for name, sign in signs.items() if sign == "negative"}
    positive_types = {name for name, sign in signs.items() if sign == "positive"}
    recognized = 0
    invalid_ids: list[str] = []
    for row in cashflow_rows:
...
def _evaluate_xirr(
    ...
    if period_provenance == "EXTRACTED" and flow_provenance and flow_provenance != {"EXTRACTED"}:
        mixed_policy = matrices.resolve(
            PRECISION_POLICY,
            "mixed_provenance_comparison",
            context="irr_comparison",
        )
        if mixed_policy == "skipped":
            return _skip(
        ...
        raise matrices.MatrixError(
            f"{PRECISION_POLICY}: unsupported mixed_provenance_comparison {mixed_policy!r}"
        )
    ...
    for cashflow in cashflow_rows:
        ...
        if cashflow_type in XIRR_EXCLUDED_CASHFLOW_TYPES:
            continue
...
def _evaluate_provenance(row: Mapping[str, str]) -> dict[str, str]:
    ...
    requirements = {
        provenance: (field,) for provenance, field in matrices.mapping(PROVENANCE_RULES).items()
    }
...
def _numeric_result(
    ...
    displayed_difference = Decimal(actual_text) - Decimal(expected_text)
    return {
        "status": "PASS" if passed else "FAIL",
        "actual_value": actual_text,
        "expected_value": expected_text,
        "difference": str(displayed_difference),
...
def _format_number(value: float) -> str:
    if abs(value) < float(matrices.resolve(RESULT_ROUNDING, "zero_epsilon")):
        value = 0.0
    return format(value, matrices.resolve(RESULT_ROUNDING, "number_format"))
