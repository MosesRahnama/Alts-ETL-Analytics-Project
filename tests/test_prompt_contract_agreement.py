...
class PromptContractAgreementTests(unittest.TestCase):
    def test_incremental_review_preserves_completed_documents(self) -> None:
        for route in ROUTES:
            for name in ("03-ADJUDICATOR-J1.md", "04-ADJUDICATOR-J2.md"):
                text = (PROMPT_ROOT / route / name).read_text(encoding="utf-8")
                self.assertIn("Existing `records-final.csv` or `coverage-final.csv` files require `validate-final` first", text)
                self.assertIn("preserve its candidates, comparisons, and decisions, then skip it", text)
                self.assertIn("only when it has no existing final", text)

    def test_foundation_investment_tables_have_supported_families(self) -> None:
        expected = {"financial_statement_observation", "position_observation", "allocation_observation"}
        self.assertTrue(expected <= set(DOC_TYPE_FAMILIES["Foundations_Annual"]))
        self.assertTrue(expected.isdisjoint(DOC_TYPE_FAMILIES["Stewardship_Proxy_Report"]))
        for family in expected:
            self.assertIn("metric_value_raw", FAMILY_CONTRACTS[family].allowed_fields)
    ...
    def test_every_named_field_is_a_real_column(self) -> None:
        ...
        controlled = (set(SUBJECT_TYPES) | set(SOURCE_STRUCTURE_TYPES)
                      ...
                      | {value for values in QUALIFIER_DIMENSIONS.values() for value in values})
    ...
    def test_working_memory_checklist_is_intact(self) -> None:
        ...
        for prompt in extractor_prompts():
            text = prompt.read_text(encoding="utf-8")
            block = re.search(r"EXTRACTION CHECKLIST.*?\n```", text, re.S)
            ...
            claimed = re.search(r"(\d+) required rules", body)
            self.assertIsNotNone(claimed, f"{prompt.name} checklist has no count")
            self.assertEqual(int(claimed.group(1)), len(numbers),
