...
`--published-slice` renders the published extraction documents.
`--manifest-only` indexes their pages without rendering and refuses missing images.
"""
...
def write_published_manifest(dpi: int = DEFAULT_DPI) -> int:
    ...
    missing = [row["image_path"] for row in rows if row["present"] != "1"]
    if missing:
        raise FileNotFoundError("Published page images are missing; render --published-slice first: " + "; ".join(missing[:5]))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--published-slice",
        action="store_true",
        help="render only the published extraction documents",
    )
    parser.add_argument(
        "--manifest-only",
        action="store_true",
