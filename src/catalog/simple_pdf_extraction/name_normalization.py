...
STATUS_RULES = "name-status-policy"
SUBJECT_RULES = "subject-kind"
COLUMN_RULES = "column-kind"
NOISE_RULES = "legal-noise"
NON_NAME_RULES = "non-names"
MATCH_RULES = "name-match-key"
MANAGER_RULES = "manager-auto-settle-rules"

DECIDED = {
    row["input_value"]
    for row in matrices.load(STATUS_RULES)
    if row["output_value"] == "settled"
}
assert DECIDED == {"decided", "auto"}
...
SUBJECT_KIND = {
    key: value for key, value in matrices.mapping(SUBJECT_RULES).items()
}
assert SUBJECT_KIND == {
    "fund": "fund",
    "manager": "manager",
...
COLUMN_KIND = matrices.mapping(COLUMN_RULES)
assert COLUMN_KIND == {"manager_name": "manager", "investor_name": "lp"}
...
LEGAL_NOISE = re.compile(matrices.resolve(NOISE_RULES, "legal_noise_pattern"))
assert LEGAL_NOISE.pattern == (
...
NON_NAMES = set(matrices.mapping(NON_NAME_RULES))
assert NON_NAMES == {"REDACTED", "NOT_DISCLOSED", "NOT_PRINTED", "N/A", "NA", "-", "--"}
...
def autofill() -> int:
    ...
    name-match-key.csv: a unique match key is auto-settled, a shared one queues.
    ...
    for kind, (path, raw_col, std_col, id_col) in KINDS.items():
        ...
        changed = 0
        if matrices.resolve(MATCH_RULES, "shared_match_key") != "queued":
            raise ValueError("name-match-key.csv names an unknown rule")
        for group in by_key.values():
            ...
            if (row.get("decision_status") or "").strip():
                continue
            if matrices.resolve(MATCH_RULES, "unique_match_key") != "auto_settled":
                raise ValueError("name-match-key.csv names an unknown rule")
...
def propagate_managers() -> int:
    ...
    for fund, (manager, source, key) in sorted(settled.items()):
        ...
        inherited_source = (
            source
            if key == fund
            else matrices.resolve(MANAGER_RULES, "inherited_source_format").format(key=key, source=source)
        )
        if existing_manager:
...
def auto_settle_manager_queue() -> int:
    ...
    if matrices.resolve(MANAGER_RULES, "identical_strings") != "auto_settled":
        raise ValueError("manager-auto-settle-rules.csv names an unknown rule")
