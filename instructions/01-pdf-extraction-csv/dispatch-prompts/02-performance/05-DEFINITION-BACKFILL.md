# Performance: DEFINITION BACKFILL (lane A or B)
...
- **Files:** `ledgers/working/pdf-extraction-csv/02-performance/<file_id>/records-<lane>.csv` (rewrite in place; the header is the current 47-column contract)
...
```powershell
python instructions/01-pdf-extraction-csv/workflow.py validate-candidate --route 02-performance --file <file_id> --agent <lane>
python instructions/01-pdf-extraction-csv/workflow.py audit-file --route 02-performance --file <file_id> --agent <lane>
```
